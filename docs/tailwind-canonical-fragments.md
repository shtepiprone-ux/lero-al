...
- **Responsive wrap rule (Task 301, 2026-05-30):** For 2-tab containers (AdminListingsTable, AdminUsersTable), use `w-full md:w-fit` + `flex-wrap md:flex-nowrap` on the pill container. At <768px this gives a full-width wrapping control (no clipping); at 768+ `md:flex-nowrap` + `md:w-fit` restores the tight single-row pill. AdminSettings already uses bare `flex-wrap` (5 tabs — acceptable without `w-fit` breakpoint). Do NOT use plain `w-fit` alone on 2-tab narrow-breakpoint admin containers.
