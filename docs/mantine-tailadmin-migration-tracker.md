...
**✅ QUEUE CLOSED — verified 2026-07-05 against live code (`theme.ts` + `input-chrome.css`), not reports:** Textarea min-height → Task 528 (`input-chrome.css` 44px class rule, TextareaAutosize-guard-safe); Popover/Menu/NavMenu shadow → Task 528 + 530 (`theme.shadows.lg` override); Card border gray-2 → Task 527 (`theme.ts` `Card.root` default-border); Select color gray-8 + disabled two-part + `[data-error]` → Task 508 + 527 (`input-chrome.css:114/143-160/66-70`); SegmentedControl active-label gray-9 → Task 527 (`input-chrome.css:307`) + inactive/hover gray-5/gray-7 → Task 539; Button secondary (white/gray-7/gray-2/shadow-xs) → Task 527 (`theme.ts:209`); Badge 12px/500/18px → Task 528 (`theme.ts:368`); input padding 10/16 → Task 527 (`input-chrome.css:104-131`); Modal radius 8px (`lg`) → Task 527 (`theme.ts:400`). **Former "pending" items resolved:** Switch unchecked track = §6h **authoritative PASS-AS-APPROVED** (`var(--color-input)`/#DEDEDE — §6h resolved the gray-200/300 ambiguity to the legacy token); Tabs active/inactive text-color = Task 541 segmented/pill redesign (`input-chrome.css:331-382`). **No open correction slice remains. Sprint 40 audit slice (Task 525) is fully closed. Next work = Phase 2 (consumer/composite migration).**
