...
const meta: Meta<typeof MantineDataTableToCards> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: [
          'Responsive data display: stacked cards (<sm / 640px) → Mantine Table (≥sm).',
          'Viewport and locale switched via Storybook toolbar.',
          '',
          '## Card config (`card` prop)',
          'When `card` is provided, mobile renders the **designed admin card anatomy**:',
          '- **Header:** id (muted) | actions (≥44px)',
          '- **Primary row:** avatar + title (medium fw=500) + subtitle (dimmed) | badge (top-aligned)',
          '- **Meta rows:** ONE divider above, edge-anchored label left / value right. No per-field dividers.',
          '',
          'When `card` is omitted, falls back to the generic 38%/62% aligned label:value layout.',
          '',
          '## Table (desktop)',
          'Per-column `align` + `width` via `TableColumn`. `verticalSpacing="sm"`, `horizontalSpacing="xl"`.',
          '',
          '**Spacing rule (§7.1):** all spacing via theme tokens — no raw px.',
          '',
