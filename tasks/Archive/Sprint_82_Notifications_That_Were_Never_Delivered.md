...
| **882** | The notification bell updates live, without a page reload: `useNotifications` subscribes only for the signed-in user (filtered), logs every Realtime status, refetches on reconnect and on tab return; an owner-run two-armed live probe | **P1** | **Q4** | — (no shared file with 878/880) | ✅ `APPROVED WITH NOTES` 2026-09-25 (review 2; archived; O82-4 passed: arm A 716 ms, arm B none; O82-5 confirmed by the owner after deploy) → [`…Task_882…`](../Archive/Sprint_82_kickoff_prompt_Task_882_Notification_Bell_Updates_Live.md) |
