...
| **871** | Moderator permission checks can never succeed: `roleHasPermission` (`src/lib/auth/permissions.ts:10-16`) reads `role_permissions` through the user-scoped client, and `authenticated` has had no `SELECT` on it since Task 275 | **P2** | Q4 | — | ✅ `APPROVED WITH NOTES` 2026-09-25 (review 1; archived — both reads on `createAdminClient()`, fail closed with one log; pre-fix arm and two plants fail as named; build exit 0; O80-4 ✅ confirmed on production; ledger `docs/reviews/2026-09-25-task871-moderator-permission-reads.review-ledger.json`) — owner route (a), 2026-09-23: *"Задача 871 - я вибираю варіант а) читати role_permissions через service role, права в базі не змінюються."* Scope also covers `getModeratorPermissions` (`/admin/permissions`), which reads the same table the same way (kickoff F5/F6) → [`…Task_871…`](../Archive/Sprint_80_kickoff_prompt_Task_871_Moderator_Permission_Reads_Through_Service_Role.md) |
| **881** | `notifications` least-privilege: the live grid (2026-09-24, owner-run) gives `anon` and `authenticated` **every** privilege on `public.notifications`, while `scripts/grant-discipline-audit.sql:158-160` declares `select` to `authenticated` and `all` to `service_role` — and that declaration is itself wrong, because `markNotificationRead`/`markAllNotificationsRead` (`src/modules/notifications/lib/mutations.ts`) update through the user-scoped client and need `UPDATE` | P2 | Q4 | — | ✅ `APPROVED WITH NOTES` 2026-09-25 (review 4; archived — O80-5 applied on production: AFTER audit N1=1/N2=1, AFTER probe exit 0, mark-as-read via `UPDATE (is_read)` confirmed; fail-closed `check:notifications-grants` in CI; ledger `docs/reviews/2026-09-25-task881-notifications-least-privilege.review-ledger.json`) → [`…Task_881…`](../Archive/Sprint_80_kickoff_prompt_Task_881_Notifications_Least_Privilege_Grants.md) |
| **883** | The bell issues one anonymous `notifications` read on sign-out: `useNotifications.ts` calls `fetchAll()` before its `userId` guard, which since Task 881 returns **401** and logs `[notifications] fetch failed` | P3 | Q4 | 881 | ✅ `APPROVED WITH NOTES` 2026-09-25 (review 2; archived — owner-native build exit 0; O80-6 ✅ confirmed on production 2026-09-25) — `KICKOFF FILED` 2026-09-25 (reserved the same day by 881's review, finding F3; owner-observed at O80-5 step 10). Hosted here by discovery, not goal fit; the owner may move it. Guard in `fetchAll`, clear on sign-out, drop stale responses; owner check **O80-6** → [`…Task_883…`](../Archive/Sprint_80_kickoff_prompt_Task_883_Notification_Bell_Never_Reads_Without_A_User.md) |
...
| **O80-4** | Task 871. **Step 1 ✅ done 2026-09-25 (owner):** census = 1 moderator (not deleted), 8 keys allowed and 3 denied, `authenticated` SELECT false. Before 871 those 8 grants had no effect; the moderator arm is reachable (`docs/sessions/evidence/task871/20-o80-4-census.txt`). **Steps 2–3 ✅ done 2026-09-25 (owner, after the deploy):** `/admin/permissions` shows the stored values and a toggle survives a reload; the moderator gains and loses the `/admin/reports` delete control with `reports.delete`. **O80-4 complete.** After approval and deploy: as admin, check `/admin/permissions` shows the stored values and a toggle survives a reload; if a moderator account exists, check a granted capability appears for it. Exact steps: 871 kickoff §13.3. |
| **O80-6** | ✅ **Done 2026-09-25 (owner).** Task 883 on production: signed in, the bell read returned 200 and the Realtime socket opened (101); after `logout` (204) no `notifications?select` request, and the Console has no `[notifications] fetch failed`. The remaining Console lines (`__cf_bm` cookie on the websocket host, font-preload and `layout-shift` warnings) are not from the bell. Evidence: `docs/sessions/evidence/task883/19-o80-6-owner.txt`. |
