...
| **O79-2** | Run `scripts/task-867-verify.sql` and return its grids (grant, policy, positive arm, negative arm). **2026-09-23: (c)/(d) PASS (anon 3 published, 0 of 1 draft). (a) FAILED — extra REFERENCES/TRIGGER/TRUNCATE pre-dating 867; (b) showed a duplicate legacy policy → kickoff §16.7; grids (a)/(b) re-run owed after the revoke.** |
