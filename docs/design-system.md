...
3. **A measured external prefix/exact-name list** — variables a framework supplies at build time that
   ...
   | `--default-transition-timing-function` | exact name | Tailwind v4's own base easing variable — `node_modules/tailwindcss/theme.css:493`, also in the production build. Was consumed as the fallback arm of a nested `var(--tw-ease, var(--default-transition-timing-function))` at `MobileBottomNavView.module.css:92`, deleted by Task 787 (2026-09-04); no live call site remains in `src/`, kept defensively |
...
is a fixed-manifest ownership check — not a route-graph parser, not a directory walk. **Twelve
...
- Eleven migration inputs: `src/app/[locale]/layout.tsx`, `src/app/[locale]/page.tsx`,
  `src/components/layout/{FooterView,HeaderView}.module.css`,
...
- `src/components/layout/MobileBottomNavView.module.css` was a twelfth migration input until Task
  787 (2026-09-04) deleted the file (mobile bottom bar removal); the manifest and its approved
  target signature were updated in the same task — see the script's own header comment for the
  current `FULL_CENSUS_*`/`MIGRATION_TARGET_*` values.
