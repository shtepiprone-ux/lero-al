...
**Opened:** 2026-08-30 · **Status:** 🟠 **OPEN** · **Landed tasks:** 4 (775 · 776 APPROVED WITH NOTES 2026-08-31 · 777 COMPLETE 2026-09-01 · 778 APPROVED WITH NOTES 2026-09-02)
...
| **778** | `/listings`: `ListingsFilters` → Mantine, hosted by the canonical `MantineDrawer` | **P2** | **Q3** | ✅ **APPROVED WITH NOTES** 2026-09-02 — archived. Evidence: `docs/sessions/2026-09-01-task778-listings-filters-mantine.md`; isolated-worktree differential `P \ B = ∅`, all 16 new canonical story cells PASS, and the required code and visual gates passed. `ListingsShellView` remains deliberately unenrolled as a mixed legacy composite until its final slice. The accepted canonical changes are recorded rather than claimed visually neutral, including the desktop drawer close affordance absent from the legacy `Sheet` (`showCloseButton={false}`). |
| **779** | `/listings`: `ListingsFilterBar` → Mantine, route visibility moved to a `ListingsShellView` wrapper | **P2** | **Q3** | 🔵 **SUPERSEDED by 780** — implemented, then returned by review; its code lives in the same uncommitted diff and is carried by 780. Original kickoff `Sprint_68_kickoff_prompt_Task_779_ListingsFilterBar_Mantine.md`; session `docs/sessions/2026-09-02-task779-listings-filter-bar-mantine.md`. Filed 2026-09-02. `Sprint_68_kickoff_prompt_Task_779_ListingsFilterBar_Mantine.md`. Migrates the horizontal filter bar off the shadcn `Button` and the legacy `Combobox` onto Mantine `Button`/`Group`/`Divider`/`Indicator`/`MantineCombobox`, consuming `LocationCombobox` as an already-migrated leaf. Pre-edit census, **with its measuring command pinned in the kickoff after a first design pass was stopped on a census-method drift**: `physical lines (Get-Content | Measure-Object -Line) = 135`, 13 `className`, 4 `<Button`, 1 `<Combobox` (`119` is the non-blank count and is explicitly **not** a baseline). The bar's `hidden md:flex` moves into a thin `<Box visibleFrom="md">` in `ListingsShellView` (`theme.ts:177` `md: '48em'` = the same 768px), so the new canonical story `Patterns/Mantine/ListingsFilterBar` (+16 cells, manifest 21 → 22) renders real UI at 320/375/390 instead of 12 contentless cells. `Indicator` is **forced, not preferred**: Mantine `Button`'s root carries `overflow:hidden` (`Button.css` `.m_77c9d27d`), which already clipped one corner badge in Task 567 round-1. `ListingsShellView` gains the wrapper but is **not** enrolled. Bound by **D68-2**. |
| **780** | `ListingsFilterBar` review rework: withdraw the production inset, absorb the `Indicator` overhang in the story harness | **P2** | **Q3** | 🟠 **KICKOFF FILED** 2026-09-02 — `Sprint_68_kickoff_prompt_Task_780_ListingsFilterBar_ReviewRework.md`. First attempt returned **`NEEDS REVISION`** (Opus, 2026-09-02): it fixed two real defects (comboboxes collapsed to content width; desktop right-alignment) but added `me="sm"` to the root `Stack` — a 12px **production-visible** right inset introduced to satisfy a Storybook-only zero-gutter condition, with no provenance, no CC row and no capture of the production consumer; `V2`-`V6` transcripts predated the final source write; no review ledger existed. **Owner decision 2026-09-02:** remove `me="sm"` from production, absorb the overhang at the `Indicator` or in the harness — the kickoff selects the harness (`preview.tsx:119-124`, `ListingsPageFrame.tsx:78`) because the `Indicator` route needs a raw-px `offset` that CC4/Task 777/R12 forbid. **This row and the kickoff were written in the same edit** (§ lines 7-10): the number had been allocated in `docs/backlog.md` on 2026-09-02 without one, which this closes. |
