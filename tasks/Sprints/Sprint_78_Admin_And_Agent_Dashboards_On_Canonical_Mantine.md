...
| **861** | **`RangeDatePicker`'s trigger is a `readOnly` input inside a click-only `MantinePopover`, so the canonical date-range picker cannot be opened by keyboard** (WCAG 2.2 SC 2.1.1). Measured 2026-09-20: `MantinePopover.tsx:115-122` and `:137` attach `onClick` only, no `onKeyDown`/`onKeyUp` in either file; `RangeDatePicker.tsx:836-838` is the `<TextInput readOnly>`. Live today in the listings date-range filter — critical-flow row "Listings date-range filter" — and inherited by 846's `MantineDashboardPeriodControl`. **Owner-bound fix: a semantic `<button type="button">` trigger with `aria-expanded`/`aria-haspopup`, NOT an `onKeyDown` on the shared popover** (it would double-toggle the native-button consumers). Acceptance is the whole keyboard-only flow: open, select, validation feedback, close, focus returns. | **P1** | Q4 | — | 📝 **KICKOFF FILED 2026-09-20 — READY FOR SONNET. PROMOTED: gates 846's approval and Wave B.** Hosted here by owner decision 2026-09-20 (discovery, not goal fit — same basis as 860) → [`…_Task_861_…`](Sprint_78_kickoff_prompt_Task_861_Keyboard_Operable_RangeDatePicker_Trigger.md) |
