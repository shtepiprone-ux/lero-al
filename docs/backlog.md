...
- **Task 629** — migrate the remaining `HeaderView.tsx` **chrome** (the `<header>` shell + container row + logo `<Link>` + desktop `<nav>`/`NavLinks` + desktop UserMenu wrapper) from raw Tailwind HTML to Mantine `Box`/`Group`/`Anchor`/`Text`; behavior controls (LocaleSwitcher/HeaderActions/UserMenu/MobileNavDrawer/hamburger) already Mantine and out of scope. Zero-pixel-change constraint; preserve Task 590 sub-390 wrap + the P0 header-hydration id-parity flow (`npm run test:header-hydration-id-parity`). Q3 visual + Q4 critical-flow regression. Two owner open questions (semantic `Box component="header"` vs `AppShell`; keep legacy tokens verbatim vs remap). 📝 KICKOFF READY — not executed. Kickoff: `tasks/kickoff_prompt_Task_629_HeaderView_Chrome_Mantine_Migration.md`.
