...
| **873** | The password-form family on canonical Mantine: `PasswordRequirementsHint` moves to `patterns/` and the duplicate rows in `Mantine/Primitives/PasswordInput` go; `ResetPasswordClient` and `CabinetPasswordSection` split into container + View on native Mantine `PasswordInput`/`Alert`/`Button` (per `AuthSheet`); the auth card is extracted from `MantineAuthFormPattern` | **P2** | **Q4** | 872 | 🟡 `PARTIALLY VERIFIED` 2026-09-24, review 2. Revision 1 is verified; approval waits on the owner's **O81-3** matrix. Review 1 found four defects, all fixed in revision 1: raw `gap={6}` in both Views; the cabinet reveal toggles are `aria-hidden` and unlabelled; Mantine drops the new-password → hint `aria-describedby` (P0 per **D81-5**; the hint becomes the input's Mantine `description`); the AC1 artifact is missing. Re-enter at kickoff §16 → [`…Task_873…`](Sprint_81_kickoff_prompt_Task_873_Password_Form_Family_On_Mantine.md) |
