...
- **Task 648 (block B, Story-first slice 1/2)** — create canonical `PopularLocationsView` Mantine component (`src/modules/locations/components/`, prop-driven, server-compatible like FooterView) reproducing the section (heading + 2→3→4-col city image-card grid: `AppImage`+overlay OR fallback gradient + `MapPin` + name) on Mantine `Title`/`Text`/`Box`/`SimpleGrid` + kept `AppImage`/`CITY_GRADIENTS`/hover-focus classes; + canonical Storybook story (both image + fallback branches) + coverage. Does NOT touch `PopularLocations.tsx`. Q3. Kickoff `tasks/kickoff_prompt_Task_648_PopularLocationsView_Mantine_Component_And_Story.md`. **Execute next.**
- **Task 649 (block B slice 2/2)** — refactor `PopularLocations.tsx` (keep the Supabase query/localization/href + null-guard) to render `<PopularLocationsView/>`, remove the inline legacy markup; rendered `/{locale}` proof. Not yet designed — depends on 648's component API.
...
**Task numbering — last used: 643 (committed); 641 applied + closed; 642 approved + closed (company-field cluster done). 644/645 done (How-it-works). 646 (block A) + 647 (block C) approved + committed. 648 designed (execute next); 649 planned (block B, PopularLocations). Next free: 650.
