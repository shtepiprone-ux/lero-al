...
| 707 | **`KICKOFF FILED`** — Sprint 48 | **Homepage tail de-Tailwind** (`tasks/Sprints/Sprint_48_kickoff_prompt_Task_707_HomepageTail_DeTailwind.md`) — `FeaturedListingsView` + `LatestListingsView` loading skeletons (**4** raw sites) + `AgentCtaButton` icon (**D31**, 1 site). Markers `featured-listings`/`latest-listings` have **0** live consumers, preserved verbatim. Proof: the **32** enrolled `Patterns/Mantine/HomepageListingGrids` cells (`Default`+`Loading`, Task 668's additive enrolment) + `check:homepage-grid`. Q3 — registry row 57 targets the populated card (Sprint 46), not this chrome. Tokens stay **23**. |
