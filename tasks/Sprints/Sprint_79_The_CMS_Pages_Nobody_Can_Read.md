...
| **O79-2** | Run `scripts/task-867-verify.sql` and return its grids (grant, policy, positive arm, negative arm). **2026-09-23: only PART (d2) came back (=0, run as one batch); PARTS (a), (b), (d1) still owed, one at a time — and d2=0 makes O79-3 required.** |
