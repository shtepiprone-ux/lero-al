/**
 * /api/cron/saved-searches auth boundary — Task 851 (Sprint 78).
 *
...
function makeRequest(method: 'GET' | 'POST', authHeader?: string): NextRequest {
  ...
  return new NextRequest('http://localhost/api/cron/saved-searches', { method, headers })
}

describe.each([
  ['GET', GET],
  ['POST', POST],
] as const)('saved-searches %s auth', (method, handler) => {
