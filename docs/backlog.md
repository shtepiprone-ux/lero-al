...
**Task numbering — last used: 429 (mojibake gate rework — signature coverage + allowlist fix). Epic II uses reserved 316–323.** **Next free (new): 430.**
...
| **BB — Listing Inquiries: Report & Message** | **243** ✅ APPROVED (commit emitted, pending owner run + SQL) (242 ✅) | BB.2 listing→owner inquiry — Path A shipped: new `listing_inquiries` table (idempotent SQL in session log), `submitListingInquiry` action (server-side owner-email resolve, IP rate-limit, typed errors), `listingInquiry.ts` Resend sender, shared `ListingInquiryDialog` (full-width bottom sheet <640), desktop + mobile triggers. Rework closed SQL idempotency + `email_transient` partial-success UX + `sm:max-w-sm` sheet fix + open-dialog matrix. Log: `docs/sessions/2026-06-15-task-243-listing-inquiry-flow.md`. Kickoff: [`Epic_BB_kickoff_prompt_Task_243_ListingInquiryFlow.md`](../tasks/Epics/Epic_BB_kickoff_prompt_Task_243_ListingInquiryFlow.md). | [`Epic_BB_…`](../tasks/Epics/Epic_BB_Listing_Inquiries_Report_and_Message.md) |
