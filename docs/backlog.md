...
| 673 | **`KICKOFF FILED`** — Sprint 47 | **`FooterView` de-hybrid only** (D29 split it from `HeaderView`). 16 `className=` sites + 8 raw HTML elements → Mantine `unstyled` primitives + `FooterView.module.css`. Verified 2026-08-01: `.site-footer` has **0 consumers repo-wide**, **no** critical-flow row, and **0** of the live 28 `check:design-tokens` violations — so the total must still read 28 after. Q3. `tasks/Sprints/Sprint_47_kickoff_prompt_Task_673_FooterView_DeHybrid.md` |
...
- **In flight, described once under "Open — needs action":** 661 · 665 (no sprint — both predate the rule) · 691 (Sprint 46) · 673 (Sprint 47, kickoff filed).
