...
**Opened:** 2026-09-04 · **Status:** 🟠 **OPEN** · **Landed tasks:** 2 (787 · 788) · **Active tasks:** 0 · **Reserved:** 1 (789)
...
|---|---|---|---|---|
| **787** | Global header Mantine migration, mobile bottom-bar removal, and guest gating of "Add listing" / "Favorites" | **P1** | **Q3** | ✅ **ARCHIVED** 2026-09-05 — `APPROVED WITH NOTES` after Revision 1, committed and pushed (`1b72b26e2`). Original review note: — implementation `VERIFIED` (R1·R3–R8, AC9 live, owner visual QA accepted **D70-2**); returned on a false reference census, one live fail-closed reference and stale state records. Revision brief → kickoff §10. → [`Sprint_70_kickoff_prompt_Task_787_…`](../Archive/Sprint_70_kickoff_prompt_Task_787_Header_Mantine_Global_And_Mobile_Nav_Consolidation.md) |
| **789** | Migrate the **live** hand-rolled filter bar in `src/modules/cabinet/components/ListingsTab.tsx` (rendered by `CabinetShell.tsx:137`) onto canonical Mantine | **P2** | TBD | reserved — kickoff not yet written. Re-measured 2026-09-18: still 51 `className`, 3 raw `<button>`, 3 `@/components/ui` imports, and the local `FilterBar` constant at `:170`. Row added 2026-09-18: the table had never listed it, although `docs/backlog.md` kept the sprint open for it. (Task **790**, filed alongside 789 by 788's trace, lives in Sprint 77.) |
| **788** | Delete the unconsumed `src/components/layout` primitive barrel — `FilterBar` · `PageShell` · `Section` · `PageHeader` + `index.ts` + 4 stories | **P2** | **Q1** | ✅ **ARCHIVED** 2026-09-05 — `APPROVED WITH NOTES` after Revision 1. Original review note: — deletion `VERIFIED` (R1·R3–R7, AC1·AC3–AC8, AC6 live 200/200 with header+footer); returned on one false census classification (`docs/ui-rules.md` still prescribes the deleted `FilterBar`) and a two-row `Files Changed` gap. Revision brief → kickoff §14. → [`Sprint_70_kickoff_prompt_Task_788_…`](../Archive/Sprint_70_kickoff_prompt_Task_788_Delete_The_Unconsumed_Layout_Primitive_Barrel.md) |
