...
**2026-06-07 — Task 409 PLANNED + kickoff REVISED for GLOBAL coverage (owner directive) — Fix listing-card pricing across EVERY consumer.** Root-cause unchanged (per_sqm="€/m²" baked currency + StoryListingCard hardcoded `€`/bypassed formatPrice + atomic-wrap + variant divergence). Owner strengthened the kickoff (still PLANNED, NOT yet handed to Sonnet): now mandates (§A) a global listing-card **consumer inventory** table before edits; **RecentlyViewedSection explicitly in scope** (inventory + rendered matrix, not conditional); a **single canonical pricing renderer / no-duplicate-markup** rule; a **repo-wide** hardcoded-currency sweep with 6-way classification (not just the 2 named files); a (§B) **automated test-coverage inventory** + **non-optional** formatter/i18n/data-contract tests with required assertions; rendered proof across ListingGrid + RecentlyViewedSection + every inventory surface; +11 ACs (17–27) + a Sonnet final-report section. sq/en/uk/it + all 14 breakpoints remain mandatory. Kickoff: `tasks/Sprints/Sprint_34_kickoff_prompt_Task_409_ListingPricingCurrencyWrap.md`. Next: hand to Sonnet.
