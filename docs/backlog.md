...
**Sprint 48 — Homepage tail de-Tailwind** (`tasks/Sprints/Sprint_48_Homepage_Tail_DeTailwind.md`), holding **707**. 🟠 OPEN, opened 2026-08-03: neither 46's listing-card chain (691 → 702 → 695) nor **Sprint 47** — ✅ CLOSED 2026-08-02, 673 `135e864e7` + 706 `911852104`, see the archive ledger — fits the homepage loading skeletons + CTA icon. **D31**: `AgentCtaButton` folds into 707 rather than becoming a one-line task. Sprint 47's **D28** (mechanism-only, zero visual delta, utilities → Mantine props + colocated `.module.css`) stays in force. **46 and 48 run in parallel; sequencing is an owner call.** After 707 and Sprint 46 land, the whole `/[locale]` tree is Tailwind-utility-free at component level.
...
| 706 | **`APPROVED WITH NOTES`** — landed `911852104`, Sprint 47 ✅ | `HeaderView` de-hybrid, reviewed 2026-08-02: 11 sites → `HeaderView.module.css`; pre/post official sweeps 16/16 md5-identical, 0 fail; `check:design-tokens` 28→23; hydration gate 3/3 owner-run; 97/97/65/65 held. P3s: AC1 count misreported (9 vs 10, corrected), AC11 arms not persisted, Arm A flipped 9/16. `sessions/2026-08-02-task706-headerview-de-hybrid.md` §14 |
| 707 | **`APPROVED WITH NOTES`** — Sprint 48, reviewed 2026-08-03 | Homepage tail de-Tailwind: 5 sites → 2 colocated modules + lucide `size`. Reviewer **recomputed** all 32 `HomepageListingGrids` PNG md5s from both official run dirs: 0 mismatches pre/post and 0 vs the executor's lists; verdicts+assertion payloads identical. Reviewer-run AC1 3/3/0, tokens **23**, integrity 7/7, mojibake 0/2046, grid 260/260 both sides. AC11 arms persisted (3×32 PNGs). Build **exit 0** on an owner-native `npm.cmd run build` at review (40/40 static pages, types valid) — the executor persisted **no** transcript and its stated path was false (corrected). P3s: `p-3` hardcodes `0.75rem` where compiled output is `var(--space-3)` (N1, repo-wide — kickoff §3.6 mis-classified it, orchestrator defect); `space-y-2` loses Tailwind's `:where()` 0-specificity (N2); icon swap rem→px (N3); AC11 noise cell unresolved (N4); stale AC12 counts, 3rd recurrence (N6). `sessions/2026-08-03-task707-homepage-tail-de-tailwind.md` §10 |
| 708 | **next free** | Assign to **Sprint 46** or **48** if it fits; otherwise open **Sprint 49** first. |

- **In flight, described once under "Open — needs action":** 661 · 665 (no sprint — both predate the rule) · 691 (Sprint 46). 706/707 are landed/approved — rows above, not in flight.
