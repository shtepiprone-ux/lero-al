...
**2026-06-15 — Tasks 428+429 (mojibake/UTF-8 double-encoding gate `check:mojibake`) — APPROVED at orchestrator review; consolidated commit emitted (pending owner run).** 428 added the gate (scanner + `package.json` script + blocking CI step + path-scoped allowlist + qa-rules note + clause-14 cross-ref); 429 reworked it (22 paired signatures incl. Albanian + Cyrillic families, no bare prefixes; pruned the unjustified backlog allowlist entry). Static diff review clean; final native gate-green run requested as proof-of-record before the commit lands. **Next: owner runs the consolidated 428+429 commit; Task 426 still pending owner-run commit.**
...
**Task 429 (governance/tooling, ✅ APPROVED — folded into the consolidated 428+429 commit):** extended `scripts/check-mojibake.mjs` `SIGNATURES` with 22 new paired sequences (Albanian e-diaeresis / c-cedilla; the U+00C2 family for NBSP, guillemets, copyright, registered, degree; and Cyrillic D-/N-prefixed pairs for the common uk letters) — no bare single-char prefixes, 0 false positives vs `messages/uk.json`. Removed the unjustified `docs/backlog.md` allowlist entry; allowlist now 6 path-scoped entries (incl. the 429 kickoff + log). `docs/qa-rules.md` "Encoding hygiene" updated. Origin: Task 428 review, both defects native-confirmed. Kickoff: `tasks/kickoff_prompt_Task_429_mojibake_signature_coverage_and_allowlist_fix.md`. Log: `docs/sessions/2026-06-15-task429-mojibake-signature-coverage-and-allowlist-fix.md`.
