...
| **852** | Admin shell on Mantine: `AdminShell`/`AdminSidebar`/`AdminMobileHeader`/`AdminLocaleSwitcher` through an extended `MantineAppShellFoundation` | P1 | Q3 | — | 🔁 NEEDS REVISION 2026-09-26 (review 8: R27–R31 accepted; drawer footer below the fold (AC11) — EXTEND `MantineDrawer`'s flex-column rule) → [`…_Task_852_…`](Sprint_78_kickoff_prompt_Task_852_Admin_Shell_Mantine.md) §23 |
