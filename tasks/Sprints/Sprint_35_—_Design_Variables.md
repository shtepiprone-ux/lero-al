...
| **404** | Refactor `src/components/shared/**` + `src/components/layout/**` to tokens (policy A). Owner-approved token promotion `--text-2xs` (micro-label, conditional swap); `z 9999` kept as justified exception (no `--z-max`) | UI/styling | KICKOFF READY | `Sprint_35_kickoff_prompt_Task_404_SharedLayoutTokenRefactor.md` |
