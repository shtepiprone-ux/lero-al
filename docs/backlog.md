...
**2026-06-15 — Task 426 (governance — canonical-first / no-duplicate-class AC clause) — ✅ APPROVED, commit emitted (pending owner run).** Adds the "Canonical-first / no-duplicate-class AC" rule at `docs/ai-behavior.md:862` ("Canonical Task Template" → "Rules") + a matching "Canonical-first respected (Task 426)" checkbox at `docs/orchestrator-role.md:237`. Native review (HEAD=`91251b6`): the clause + backlog entries already landed in `c4a711ab4`; the review commit lands `orchestrator-role.md` + the session log. Orchestrator corrected the session log's AC#3/Files-Changed to native truth at review. Log: `docs/sessions/2026-06-15-task426-canonical-first-ac-clause.md`. **Next (new): Task 428 — mojibake/UTF-8 encoding guard (kickoff opened).**
...
**Task numbering — last used: 428 (mojibake/UTF-8 encoding guard). Epic II uses reserved 316–323.** **Next free (new): 429.**
...
**Task 428 (governance/tooling, OPEN — not started):** add a `check:mojibake` gate that scans tracked text files for UTF-8-as-CP125x double-encoding artifacts (`Ô£à`→✅, `ÔåÆ`→→, `ÔÇö`→—, `â€¦`, replacement char `�`, …) and FAILS CI + the native pre-commit hook on any hit, with an allowlist for docs that legitimately quote the artifacts (this session log, the 428 kickoff). Plus a docs note requiring UTF-8 PowerShell console (`chcp 65001`) and forbidding piping console output into repo files. Origin: 2026-06-15 owner report of mojibake in PowerShell paste (files were clean; risk is console encoding leaking into files). Kickoff: `tasks/kickoff_prompt_Task_428_mojibake_encoding_guard.md`.
