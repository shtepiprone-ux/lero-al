...
|---|---|---|---|---|---|
| **763** | 1 | `AppImage` de-Tailwind: 21 variant strings + 4 inline strings + the `group`/`group-hover` pair | 63 | P1 | **`NEEDS REVISION`** 2026-08-21 — 20/21+4 strings migrated and accepted; 1 P1 (utility-shaped class names, kickoff §10.3.2). Revision 1 is rename-only: `Sprint_63_Task_763_revision_1_class_naming.md`. `listing` hover correctly `BLOCKED`, moved to Phase 2 under D63-E |
| **764** | 2a | Fold `listing`'s hover into the card pattern (D63-E route c) — measure the coarse-pointer and transition-curve deltas first, then delete the last Tailwind utility from the homepage card | 63 | P1 | **KICKOFF FILED** 2026-08-21 — `Sprint_63_kickoff_prompt_Task_764_Listing_Hover_Fold.md`. **Blocked until 763 Revision 1 is `APPROVED`** (both edit `appImageConfig.ts`) |
| *next free* | 2b | `MantineListingCardPattern.tsx:305-306` (`flex flex-col`, `grayscale opacity-60`) · `LocaleSwitcher.tsx:55` (`animate-spin`) | 63 | P2 | NOT FILED — split out of 2a so the behaviour-bearing hover change ships alone |
...
| **D63-E** | ✅ **DECIDED 2026-08-21 — route (c).** Owner: *"Route C для D63-E виглядає найкращим, але реалізовувати його треба окремою Phase 2 задачею після вимірювання coarse-pointer сценарію."* Delete `hoverClass` from the `listing` variant, set `MantineListingCardPattern.module.css:68` to `scale(1.1025)`, delete `'group'` at `:304`. Routes (a) accept-the-regression and (b) permanent-debt are **rejected**. | Filed as **Task 764** (`Sprint_63_kickoff_prompt_Task_764_Listing_Hover_Fold.md`), separate from 763 per the owner's instruction, with the coarse-pointer measurement as its blocking Phase A and a **pre-declared** outcome so the measurement cannot fork the plan. Filing also surfaced a second never-equivalent dimension the decision did not know about: `scale` is absent from `.imageSection img`'s transition list (`:53`), so today the zoom snaps to 1.05x and only then eases to 1.1025x — Phase A2 samples mid-flight, because a rest/settled comparator would report zero delta while the animation shape changed. |
| **D63-F** | `check:design-tokens --strict` is red on exactly two declarations, both in `AppImage.module.css`: `:116 border-radius: 3.40282e38px` and `:150 transition-duration: 300ms`. Accept as a standing recorded `CONFLICT`, or resolve — and if resolve, how? | Sprint 63 rule 2 forbids the same-line `design-tokens-allow:` marker, and a path-level `scripts/design-tokens-allowlist.json` entry is the same class of author-applied exemption, so neither is available without an owner exception. The third route is to declare both values as real tokens in `globals.css`'s **`:root`** block (not `@theme inline`) — which is Phase 5's territory. **Both literals are forced, not sloppy:** `rounded-full` compiles to that raw float, and `--duration-slow`/`--ease-standard` are declared in `@theme inline` but emitted by no built stylesheet. |
