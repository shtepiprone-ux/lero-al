...
| **793** | The contact card becomes canonical Mantine, moves into the content flow, and takes its clearance with it | **P1** | Q3 | 🟡 **KICKOFF FILED — EXECUTABLE**, **rewritten clean 2026-09-06**. All four owner decisions folded into the body: card in flow below `lg`, fixed bar **deleted**, favorite **and** share into the badges row. Deletes Task 791's `listingContactBarClearance` and its consumers in four files — one of which `throw`s when the token is missing. Reverses **Task 466**'s bar consolidation and finally makes production match **D69-25**. → [`Sprint_71_kickoff_prompt_Task_793_…`](Sprint_71_kickoff_prompt_Task_793_ListingContact_Canonical_Mantine_Card.md) |
