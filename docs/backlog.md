...
**2026-07-05 — Owner P0 rule change (Variant A) + HOLD on the three zip-absent primitives.** Owner stopped the Task 549 rendered-gate step: the themed Toast renders as stock Mantine, not TailAdmin. Root cause found + evidenced: Toast (549), Slider (548) and Skeleton (544) are all "honest-negative on the zip" (`demo_tailadmin_com.zip` has zero markup for them) and were each closed by formalizing prior live-measured PROSE into a §6x row + accepting Mantine defaults ("zero-override: default already matches"). The crash-and-geometry rendered gate (Task 529) never checks TailAdmin chrome, so it stayed green. **New agent-contract clause 16a (2026-07-05):** zip-absent primitives MUST be re-grounded on a LIVE `demo.tailadmin.com` capture (screenshot + `getComputedStyle`, recorded with provenance) and every value — incl. each zero-override — positively verified side-by-side; "formalize prose + Mantine default" closures are now a task failure. Codified in `agent-contract.md` §16a + `orchestrator-role.md` "TailAdmin conformance gate" + `tailadmin-style-reference.md` §6r preamble. **544/548/549 → HELD/reopened** pending Task 550. **Next:** Task 550 live-reference re-verify (Toast→Slider→Skeleton).
