...
- **Task 645** — swap the legacy `page.tsx` "How it works" block to render `<HowItWorksSteps/>`, remove the inline Tailwind markup, rendered proof on the real `/{locale}` route. Not yet designed — depends on 644's component API (now available: `{ heading: string; steps: [{title,desc}×3] }`).
