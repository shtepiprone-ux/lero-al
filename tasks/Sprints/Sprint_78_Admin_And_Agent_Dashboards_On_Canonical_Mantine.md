...
| B — data (independent of A except `period.ts`) | **851 → 849** (both edit `vercel.json`) · **850** · ~~**847**~~ · ~~**848**~~ | **847 and 848 are both approved and archived 2026-09-20**, so `BlockResult`, `getAdminDashboardData()` and `src/modules/cabinet/statistics/` are final and Wave C's 853/854 are unblocked. **851 · 849 · 850 remain**, none of them touching `period.ts`. Every one of their kickoffs still carries the vacuous `git grep` AC form — fix via **864** before execution. |
