...
**Task numbering — last used: 652. 641 applied + closed; 642 closed (company-field cluster done). 644–650 approved (homepage → Mantine chrome migration COMPLETE). 651 doc-correction IMPLEMENTED - AWAITING REVIEW, **still uncommitted** (verified via `git log -- docs/mantine-responsive-design-system.md`: no commit past the pre-existing governance-docs commit touches the file; working tree still shows the correction as a pending diff — corrects the prior "approved + committed" line, which was inaccurate). 652 (hero §6c unification + R8 radius revision) IMPLEMENTED - AWAITING REVIEW. Next free: 653. **Governance: `npm run build` exit 0 now mandatory for non-Q0 (`67340ff49`).**
