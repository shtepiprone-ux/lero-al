...
| **878** | The header renders the real bell everywhere it is proven, and loses its inline-style hardcode. `NotificationBellView` trigger → `subtle` (the owner's Story look); the `HeaderView`/`HeaderActions` Stories render the real `NotificationBellView`, not a stand-in; 28 inline `style` objects across 9 header-tree files → Mantine props and theme tokens (D81-7) | **P2** | **Q3** | 875 | 🔍 `PARTIALLY VERIFIED` 2026-09-24 (review 3 — §17 verified; approval waits only on the §17.5 owner re-check) → [`…Task_878…`](Sprint_81_kickoff_prompt_Task_878_Header_Bell_And_Inline_Style_Hardcode.md) |
