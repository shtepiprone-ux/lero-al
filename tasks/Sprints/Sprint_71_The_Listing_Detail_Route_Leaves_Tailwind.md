...
| **792** | Detail-route chrome — `ListingBackButton`, `ListingStatusBanner`, `[slug]/loading.tsx`, the section wrappers — **plus the dead "similar listings" link** | P2 | Q2 | ⬜ reserved. **Owner-reported defect 2026-09-06:** the status banner's "similar listings" link is `href="#similar-listings"`, an in-page anchor on an already-dead listing; it must navigate to `/{locale}/listings` pre-filtered by `type` · `property_type` · `location_id`. That makes 792 a chrome migration **plus** one behaviour change — say so in its kickoff rather than smuggling it in. |
