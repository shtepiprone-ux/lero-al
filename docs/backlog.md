...
| 706 | **`KICKOFF FILED`** — Sprint 47, unblocked (673 landed) | **`HeaderView` de-hybrid** (`tasks/Sprints/Sprint_47_kickoff_prompt_Task_706_HeaderView_DeHybrid.md`) — 11 `className=` sites, **zero raw HTML elements** (Task 629 already converted them), inherits 673's module pattern. **Q4** — carries everything 673 did not: the P0 critical flow *Authenticated header hydration* (599/601), **3** live `.site-header` consumers (`check-header-id-parity.mjs:147` · `MantineRootProvider.tsx:34` · `task612-qa-listinggallery-lightbox-portal.mjs:57`), and Task 684's hardcoded **97px @320/375 · 65px @390/1024** measured off this header's own height. **D30** (2026-08-02): the **5** `min-[390px]` violations (`:110` ×3, `:128` ×2) are in scope and move into the module's own `@media` — verified that the token scanner raises no `length:` finding on any `.css`, so `check:design-tokens` ends at **23**. |
