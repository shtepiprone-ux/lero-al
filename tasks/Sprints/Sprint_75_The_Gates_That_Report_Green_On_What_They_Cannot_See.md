...
| # | Title | P | QA | State |
|---|---|---|---|---|
| **812** | `check:story-coverage` reports green for the components it cannot see | **P0** | **Q4** | **APPROVED** 2026-09-11 — R1-R20 verified against the artifacts; R20 closed by the owner directly in `docs/golden-rules.md:92`. Archived in `docs/backlog-archive.md`. GR-1/GR-3 remain **not enforced** — gated on **817** and **818**. → [`Sprint_75_kickoff_prompt_Task_812_…`](../Archive/Sprint_75_kickoff_prompt_Task_812_Rendered_But_Unenrolled_Component_Detector.md) |
...
| **816** | Design-system Mantine-pattern ownership and audit — owns the 11-path tier-3 list in Task 812 §14.6.1 | P1 | Q2 | **APPROVED WITH NOTES** 2026-09-11 — archived in `docs/backlog-archive.md`. `npm run audit:design-system-patterns` landed and correctly exits 1: **3 of the 11** allowlisted patterns measurably fail decision 1's own word *shared* (1 consumer each). The governance model for the 33 is now an **owner action row** in `docs/backlog.md`. → [`Sprint_75_kickoff_prompt_Task_816_…`](../Archive/Sprint_75_kickoff_prompt_Task_816_Design_System_Pattern_Ownership_And_Audit.md) |
| **817** | `scripts/check-surface-census.mjs --surface <path>` — GR-1's real per-surface command | **P0** | Q4 | **APPROVED WITH NOTES** 2026-09-11 after Revision 1 — archived in `docs/backlog-archive.md`. GR-1's `Command` block is runnable; the blind spot is measured (planted de-enrolment: the census blocks on `CollectionsSection`, `check:rendered-scope` names it zero times). GR-1/GR-3 remain **not enforced** — gated on **818**. 3 P3 notes carried to 816/818. → [`Sprint_75_kickoff_prompt_Task_817_…`](../Archive/Sprint_75_kickoff_prompt_Task_817_Per_Surface_Census_Command.md) |
| **818** | Make the advisory `check:rendered-scope` rollout blocking — clear the frontier or add a fail-on-new baseline | **P0** | Q4 | **APPROVED WITH NOTES** 2026-09-11 after Revision 1 — archived in `docs/backlog-archive.md`. The gate is **blocking** in CI against a versioned 29-edge baseline, with a 5-arm `check:rendered-scope:verify` self-test. GR-1/GR-3 now enforced **for the enrolled subgraph only** — exit criterion 2 is not met; owner decision 4 files **819**. → [`Sprint_75_kickoff_prompt_Task_818_…`](../Archive/Sprint_75_kickoff_prompt_Task_818_Rendered_Scope_Becomes_Blocking.md) |
| **819** | GR-1's pre-enrolment case becomes blocking — diff → affected surfaces → `check-surface-census.mjs --surface` each | **P0** | Q4 | **APPROVED WITH NOTES** 2026-09-11 after Revision 1 — archived in `docs/backlog-archive.md`. Blocking in CI against a 690-block/268-surface baseline with carried/stale/new semantics and an 8-arm self-test. **Exit criterion 2 is now met.** → [`Sprint_75_kickoff_prompt_Task_819_…`](../Archive/Sprint_75_kickoff_prompt_Task_819_Pre_Enrolment_Census_Becomes_Blocking.md) |
...
| **837** | The detail pattern Story fills its favorite slot with a bordered stand-in, and the pill shape it also proves has no consumer | P2 | Q3 | ✅ **APPROVED** 2026-09-18 (Opus review 4, Revision 1 / run3) — archived in `docs/backlog-archive.md`. Owner accepted all visual tuples 2026-09-18. Revision 1 (owner decision 2026-09-18, supersedes D69-27): badges row `align="flex-start"`; run3 capture shows favorite/share tops equal to the first badge top in every row of all 5 cells, `badgeRows` 1/2/3 at 1440/390/320. Review 3 (2026-09-18) returned the centred alignment. Review 2 (2026-09-18) verified R1-R7 on run2. Review 1 (2026-09-17) found the evidence stale and sent it to run2. Filed by the owner's visual review of 826 (screenshot of `Patterns/Mantine/ListingDetailPattern → Default` at 1440/en). Measured: `DemoFavorite` (`:39-45`, 6 consumers) renders 34×34 with `1px solid rgb(152,162,179)`; every real `FavoriteButton` render is 32×32 with a transparent border. R1 puts the real component in the slot (`ListingCardPattern.stories.tsx:193` precedent). R2 deletes the pill by **owner decision 2026-09-17** — `shape`, `size`, `PILL_SIZE_MAP`, `bd="1px solid var(--border)"`, `theme.other.radius.favoritePill` and the pill unit test; zero production consumers (Task 211 put it in `ListingContact`, 784/793 re-homed the favorite away). Binding owner constraint 2026-09-17: no hardcode, canonical Mantine, canonical tokens. R6 records the slot blind spot both gates share — Sprint 75 exit criterion 1. The site header is explicitly out of scope (owner). → [`Sprint_75_kickoff_prompt_Task_837_…`](Sprint_75_kickoff_prompt_Task_837_Detail_Pattern_Favorite_Stand_In_And_Pill_Deletion.md) |
