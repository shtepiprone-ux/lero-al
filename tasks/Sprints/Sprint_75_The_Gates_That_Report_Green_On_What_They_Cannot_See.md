...
| **831** | `check:surface-census:changed` cannot see a stale parent row when the child's own diff enrols it | P2 | Q4 | **NEEDS REVISION** 2026-09-17 (review 1) — R1-R7 landed and the 7 LightboxView rows go stale. Revision §16, by **owner decision 2026-09-17** ("не створюй нову задачу, необхідно все зробити в рамках цієї задачі"): R8 a re-censused parent missing on disk retires its rows, R9 printed rule, R10/R12 arms 11-12, R11 a `.ts` root is not a component, R13 retire the false `.ts` self rows (155 at review), AC8 pre-fix writer red evidence. → [`Sprint_75_kickoff_prompt_Task_831_…`](Sprint_75_kickoff_prompt_Task_831_Census_Changed_Retires_Stale_Parent_Rows.md) |
