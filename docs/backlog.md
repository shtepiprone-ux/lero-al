...
- **Task 623** — split public `Footer` into container + prop-driven `FooterView` (`Mantine/Primitives/FooterView` story, 2 fixtures: DB content + hardcoded fallback), then migrated markup to Mantine `Box`/`Stack`/`Group`/`Flex` (links/typography/grid-gap kept as Tailwind — no matching Mantine token for 40px/10px gaps, and unverified `Anchor`/`Text` cascade risk). Geometry-diff + DOM measurement + live link-attribute proof show before/Phase-1/Phase-2 byte-identical. `--mantine-only` gate run twice (Phase 1 + final Phase 2 code): both exit 0, 0 FooterView failures/ambiguous. Pre-existing `check:story-coverage` gap surfaced (6 uncovered Mantine-primitive-with-container components, 5 predate this task) — not fixed, flagged for Opus. ✅ IMPLEMENTED - AWAITING ORCHESTRATOR REVIEW. Session: `docs/sessions/2026-07-18-task623-footer-view-split-and-mantine-migration.md`.
