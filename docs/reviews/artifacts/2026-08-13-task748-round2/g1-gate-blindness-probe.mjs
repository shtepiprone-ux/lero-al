#!/usr/bin/env node
/* REVIEWER PROBE (round 2, finding G1) — NOT a deliverable of Task 748.
 *
 * This is docs/reviews/artifacts/2026-08-13-task748-rework/twmerge-class-resolution-all18.mjs,
 * byte-for-byte, with exactly one change: the two E*b after-side expressions have had the RR1
 * fix REVERTED, so the module class is unconditionally present again — precisely the state
 * finding F-A described and RR1 was written to eliminate.
 *
 * A regression gate that protects RR1 must exit non-zero here. It exits 0.
 * Cause: line ~89's whitelist test is unconditional on the case id
 *   c.id.includes('priority row') || c.id.includes('predictive row') || c.id.includes('photoCountButton')
 * so the three sites that matter can never contribute to unexpectedMoves.
 */
/**
...
CASES.splice(idx6, 1,
  ...
  { id: 'E6b PerfDevOverlay.tsx priority row (priorityOver=true)', before: () => cn('text-overlay-foreground/70', true && 'text-destructive font-bold'), after: () => cn(MODULE('metricRow'), true && 'text-destructive font-bold') /* <-- RR1 FIX REVERTED BY THE REVIEWER */ },
);
...
CASES.splice(idx7, 1,
  ...
  { id: 'E7b PerfDevOverlay.tsx predictive row (predictiveOver=true)', before: () => cn('text-overlay-foreground/70', true && 'text-destructive font-bold'), after: () => cn(MODULE('metricRow'), true && 'text-destructive font-bold') /* <-- RR1 FIX REVERTED BY THE REVIEWER */ },
);
