...
**🟡 Sprint 40 (TailAdmin conformance — ALL primitives) — OWNER P0, agent-contract clause 16.** Task 525 audit ✅. Task 527 ❌ REJECTED. Task 528 ✅ APPROVED + committed. **Task 529 ✅ APPROVED + committed** — Mantine/Primitives/* auto-discovered + CI-blocking in `screenshots:assert`, coverage hole closed; native gate green (332/336 PASS, 0 FAIL). Sprint: `tasks/Sprints/Sprint_40_TailAdmin_Conformance_AllPrimitives.md`.
...
**Task numbering — last used: 530 (kickoff written, unexecuted). Next free: 531.**

**🟡 Task 530 (Sprint 40 — overlay dropdown TailAdmin conformance) — KICKOFF WRITTEN, awaiting Sonnet.** Full dropdown conformance for `Popover` + `DropdownMenu` + `NavigationMenu` (one slice, Note 14). Confirmed delta: `shadow:'lg'` in theme references Mantine's DEFAULT shadow scale (no `theme.shadows` override), so it renders Mantine `lg`, not TailAdmin `shadow-theme-lg`. Scope = extract §5 `shadow-theme-lg` value from zip → single-source token override (STOP-and-ASK if a `shadow="lg"` sibling would regress) → verify/correct full container+item chrome §6l side-by-side. Mobile <640 bottom-sheet must not regress. Rendered side-by-side (incl. ≥640 shadow cell) is the style proof; `--mantine-only` gate is baseline only (crash/geometry, not shadow — §14.9.7). Kickoff: `tasks/Sprints/Sprint_40_kickoff_prompt_Task_530_OverlayDropdownConformance.md`.
