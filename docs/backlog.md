...
**Homepage → Mantine migration (audit 2026-07-20; blocks A/646 + C/647 + B/648–649 done → archive).** Block D correction: the "HeroSearch Phase-2 blocked" note was **stale** — `HeroSearchView`'s Combobox consumers (`PropertyTypeCombobox`/`LocationCombobox`) are already `MantineCombobox`, controls all Mantine, 0 legacy `@/components/ui/*`. Only the container `<div>` chrome remains:
- **Task 650 (block D)** — migrate `HeroSearchView.tsx`'s 4 container `<div>`s → Mantine `Box`(×3)/`Paper`(search surface), classes preserved verbatim (preserve-look, owner). Tabs kept as styled `Button` (NOT `SegmentedControl` — owner declined the look change); controls/`FiltersPanel`/`HeroSearch.tsx` untouched. Q3 + **mandatory `npm run build`**. Kickoff `tasks/kickoff_prompt_Task_650_HeroSearchView_Container_Chrome_Mantine.md`. **Execute next — final homepage chrome piece.**
...
**Task numbering — last used: 643 (committed); 641 applied + closed; 642 approved + closed (company-field cluster done). 644/645 done (How-it-works). 646 (block A) + 647 (block C) approved + committed. 646/647/648/649 approved (homepage blocks A/C/B). 650 designed (execute next — HeroSearch container chrome, block D; Phase-2 "blocker" was stale). Next free: 651. **Governance: `npm run build` exit 0 now mandatory for non-Q0 (`67340ff49`).**
