...
**Opened:** 2026-09-24 · **Status:** ✅ **CLOSED** 2026-09-25 (owner: *"Sprint 81 закривай."*) · **Landed tasks:** 6 (872, 873, 875, 876, 878, 879) · **Kickoffs filed:** 0 · **Reserved:** 0 · **Moved out:** 874 → Sprint 78 (D78-7, 2026-09-24)
...
| **D81-7** | The owner reported (2026-09-24): *"в HeaderView Story кнопка сповіщень виглядає так як треба, а от на реальному сайті вона зовсім інша … вияснити чи існує хардкод у шапці (якщо так - видалити і замінити якісним кодом)"*. Q: how to task the audit's findings (the bell stand-in, 28 inline style objects, 4 Tailwind-reproduction CSS modules with no TailAdmin header reference)? → *"Two tasks (Recommended)"*. The option read: *"878 now: the real bell in the HeaderView/HeaderActions Stories, the bell trigger switched to the Story look (subtle, like the heart next to it), and every inline style object in the header tree replaced with Mantine props/theme tokens. 879: the four CSS modules and global classes, starting by capturing a TailAdmin header reference."* | 2026-09-24 | 878 filed (kickoff); 879 reserved. The bell's target look is the Story's `subtle` variant. |
| **D81-8** | Owner, 878 review 2 (O81-8 matrix), verbatim: *"все ок, але б я хотів, щоб counter був ближче до іконки як показано на скріншоті. Наразі я бачу цей counter далеко від іконок, допоки не наведеш мишкою не зрозуміло до чого саме відноситься цей counter"* (screenshot: a count badge overlapping the top-right corner of a heart/cart glyph). Follow-up the same day: *"цей couner з кількістю показується у багатьох сторісах, наприклад у кабінеті та у адмінці."* | 2026-09-24 | Every other O81-8 tuple is **accepted**. The unread-count `Indicator` is **returned**: its centre must sit on the bell glyph's top-right corner, not the 44px touch box's corner. The fix goes into the one canonical source (`NotificationBellView`), which every counter instance renders → 878 kickoff §17. |
| **D81-9** | Owner, 878 review 4 (§17.5 re-check), verbatim: *"в Storybook ці count overlaps не круглі!"* … *"я хочу щоб ці red count overlaps були як на Rozetka.com.ua або Prom.ua, круглі"*, with a Rozetka header screenshot. On two or more digits: *"Pill is fine (Recommended)"*. | 2026-09-24 | The badge becomes 20px (`theme.other.layout.iconButtonIndicatorSize`), because the reviewer measured any 16px circle as faceted at DPR 1 (878 §18.2); multi-digit counts stay Mantine pills; `NotificationBellView` `Default` gains 12 and `99+` sections → 878 kickoff §18. |
| **D81-6** | Q (875's design): Cloudflare Turnstile has no Albanian, so which language does the captcha get for `sq`? → *"auto for sq (Recommended)"*. The option read: *"sq passes \"auto\", Turnstile's current default: it uses the visitor's browser language if supported. Same as today for sq visitors, never worse than en. en/it/uk get their own code."* | 2026-09-24 | 875 R1: `en`, `it` and `uk` pass their own code; every other locale, `sq` included, passes `auto`. |
...
3. Every tier-1 node the header renders is enrolled with its own Story, or exempt under D81-2. The tier-2 edge is
   named and filed (873).
4. *(Added 2026-09-24 with D81-7.)* The header's Stories prove what production actually renders, with no slot
   stand-ins, and the header tree carries no hardcoded visual value (878 for the bell and the inline styles, 879 for
   the CSS modules). Sprint 70, "the site chrome leaves Tailwind", was considered and rejected: its only open item is
   789, and this work is the header's proof and quality, the same surface as goals 1–3.
...
| **874** | The last `ui/PasswordInput` consumer (`AdminExchangeProvidersManager`) moves to Mantine, then the legacy file and its Story are deleted | P3 | Q3 | 873 | ➡️ **MOVED to Sprint 78** 2026-09-24 by owner decision D78-7; its state lives in that plan |
...
| **875** | The Turnstile captcha follows the app locale: `CaptchaWidget` passes `useLocale()` to Turnstile's native `options.language` (filed 2026-09-24 by 872's O81-2 review) | P3 | Q2 | — | ✅ `APPROVED WITH NOTES` 2026-09-24, review 2 (review 1 `PARTIALLY VERIFIED` pending O81-6; the owner accepted all 8 tuples). Notes: one new lint warning (`_ref` unused in the test mock) and the platform evidence file named `00-` instead of `10-`. O81-7 is owed after deploy (D81-6: `sq` → `auto`) → [`…Task_875…`](Sprint_81_kickoff_prompt_Task_875_Turnstile_Follows_App_Locale.md) |
| **878** | The header renders the real bell everywhere it is proven, and loses its inline-style hardcode. `NotificationBellView` trigger → `subtle` (the owner's Story look); the `HeaderView`/`HeaderActions` Stories render the real `NotificationBellView`, not a stand-in; 28 inline `style` objects across 9 header-tree files → Mantine props and theme tokens (D81-7) | **P2** | **Q3** | 875 | ✅ `APPROVED WITH NOTES` 2026-09-24 (review 5; archived; landed with 880, D82-6) → [`…Task_878…`](../Archive/Sprint_81_kickoff_prompt_Task_878_Header_Bell_And_Inline_Style_Hardcode.md) |
| **879** | The header's four CSS modules (`HeaderView`, `MobileNavDrawer`, `LocaleSwitcher`, `AuthSheet`) are deleted; their chrome becomes Mantine props and theme tokens traced to the new TailAdmin §6v row (zip header + live sign-in capture, 2026-09-25). Height (97/65) and stacking (30) kept; `.site-header` stays a marker, `.container-wide` untouched (D81-7) | P2 | **Q4** (header, login and hydration critical-flow rows) | 878 | ✅ `APPROVED WITH NOTES` 2026-09-25 (review 2, Revision 1; archived; Storybook matrix O81-9 accepted) → [`…Task_879…`](../Archive/Sprint_81_kickoff_prompt_Task_879_Header_Chrome_On_Mantine_And_TailAdmin.md) |

## Owner actions this sprint needs

| ID | Action |
|---|---|
| **O81-1** | ✅ Done locally 2026-09-24 (owner, before approval); ✅ **repeated on the live site lero.al 2026-09-24** (owner: *"Все ок"*). After 872 is deployed: sign out on a listing page (stays, shows the sign-in prompt), on `/favorites` or `/cabinet` (goes to the homepage), and from the mobile menu (the drawer closes). |
...
| **O81-6** | ✅ **Accepted by the owner 2026-09-24**, all 8 tuples (*"Все ок"*). 875's `OWNER VISUAL QA REQUIRED` matrix, before approval: `Patterns/Mantine/AuthSheet` → `Captcha` × `en`/`it`/`uk`/`sq` × 320/1440, 8 tuples. For `sq` the widget shows your browser's language (D81-6). |
| **O81-7** | ✅ **Confirmed by the owner 2026-09-25** (*"все перевірено візуально, все ок"*). After 875 deploys, on lero.al: the Register widget on `/en`, `/it`, `/uk` and `/sq` matches O81-6, and one real registration or recovery request still verifies. |
| **O81-8** | ✅ **Confirmed by the owner 2026-09-25** (*"все перевірено візуально, все ок"*). 878's `OWNER VISUAL QA REQUIRED` matrix, before approval (kickoff §13.3): `HeaderView` `Default`/`SigningOut`, `HeaderActions`, `NotificationBellView`, `AuthSheet` `Login`/`Register`/`RegisterAgent`/`ForgotPassword`, and (review 1) `UserMenu`, `LocaleSwitcher`, `MobileNavDrawer`. **2026-09-24: all accepted except the bell's counter position (D81-8); after §17, re-check only the §17.5 tuples.** ✅ **Storybook part accepted 2026-09-24** (owner, after §18: *"Візуально тепер індікатор круглий"*). **Still owed after deploy:** the live checks in 878 §13.3 step 2. After deploy: the bell on lero.al, and the forgot-password success screen. |
| **O81-9** | ✅ **Confirmed by the owner 2026-09-25** (*"все перевірено візуально, все ок"*). 879's `OWNER VISUAL QA REQUIRED` matrix (kickoff §13.3, 7 rows, opened through the iframe URL at a set window width, because the toolbar viewport does not resize (799)), before approval. After deploy: the header at 390/1440, the drawer at 390 and one forgot-password success screen on lero.al. ✅ **Storybook part accepted 2026-09-25** (owner: *"Візуально все ок з цими сторісами"*, all 7 rows). **Still owed after deploy:** the live checks above. |
| **O81-3** | ✅ Storybook matrix accepted 2026-09-24 (after the §17 Story fix). ✅ **Live check done on lero.al 2026-09-24** (owner: *"Все ок"*): one real password reset and one cabinet password change. 873's `OWNER VISUAL QA REQUIRED` matrix (ResetPasswordView, CabinetPasswordSectionView, PasswordInput hint states, AuthFormPattern + AuthCard). After deploy, complete one real password reset and one cabinet password change. Note: the green/red ring around the new-password field goes, matching the canonical `AuthSheet` registration form. |
