...
## Last Session (2026-08-05) — 709 + 709-R approved and committed; D34 ratified

- **709 + 709-R `APPROVED WITH NOTES`, committed `c1f9461bc`** → archive ledger. `HeroSearchView` de-Tailwind (9 sites) plus the `@layer utilities` fix for the cascade regression it exposed; reviewer recomputed 40/40 herosearch md5s against the pre-709 baseline, 0 mismatches.
- **D34 ratified** (Sprint 49 §3) — a D28 de-Tailwind module reproduces the utility's cascade **layer**, not only its declaration and specificity. Inverse of the 602/656-family, which stays unlayered on purpose. Binds Sprints 46/49/50.
- **710 is Sprint 49's last open task**, and its scope grew: 709 persisted `EXIT_CODE=0` beside 4 genuine FAILs; 709-R proved unpiped that the gate really exits 1, so that zero was a capture artifact worth gating.
...
**Sprint 49 — HeroSearch: gate integrity, then de-Tailwind** (`tasks/Sprints/Sprint_49_HeroSearch_Gate_And_DeTailwind.md`). 🟠 OPEN — 708 ✅ · 709 + 709-R ✅ · **710 `KICKOFF FILED`, closes the sprint**. **711 needs a sprint before its kickoff** (open 51 or fold into 50). **Sprint 50** is reserved for `MobileBottomNavView` (app-shell, the Sprint 47 Header/Footer family, 3 `design-tokens-allow` markers to carry across) — open it with its own plan file, and issue its task number, before writing that kickoff.
...
## Task registry — single source for every open number. Last used **711**, NEXT FREE **712**
...
| 710 | **`KICKOFF FILED`** — Sprint 49, Q4, closes the sprint | **Meta-gate: an assertion `null` in every one of its target cells is a dead gate, not a pass.** Exactly how 573 died unnoticed for ~5 weeks. **⚠️ Census 2026-08-05 — not hypothetical: `fullWidthButtonsAtMobile` and `popupBottomSheetAtMobile` are `null` in ALL 1184 cells** of the CI-blocking `--mantine-only` matrix; `fullWidthControlsAtMobile` (852 `true`), `heroSearchWrapInBand` (4) and `noHorizontalOverflow` (1184) are live. Gate reads a persisted manifest, classifies `LIVE`/`DEAD-NEW`/`DEAD-KNOWN`/`STALE-ENTRY`, and fails on a new death **or a stale registry entry** so the registry can't rot. Exit-code half settled: the sweep does exit 1 unpiped (709-R), so 709's `EXIT_CODE=0` was a capture artifact — gated structurally + as a written capture rule. `Sprint_49_kickoff_prompt_Task_710_Assertion_Liveness_Meta_Gate.md`. |
| 711 | reserved, blocked on 710 — **sprint not yet assigned** | Re-anchor `fullWidthButtonsAtMobile` + `popupBottomSheetAtMobile` onto Mantine DOM. One root cause: their candidate selectors are shadcn `data-slot` names (`check-stories-rendered.mjs:1161`, `:1185-1192`) emitted only by `src/components/ui/*`, which Mantine-scope stories never render — the same defect governance §14.9.9 recorded for geometry's `PORTAL_SELECTOR`. Needs a planted proof per assertion. Open Sprint 51 or fold into 50 before writing the kickoff. |
