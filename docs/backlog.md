...
**2026-07-08 — Task 558 `RangeDatePicker` REJECTED by owner → Task 561 fixes opened — orchestrator review.** Reviewed the real `RangeDatePicker.tsx` diff (not the report); confirmed all 6 owner defects: (1) duplicate mobile month/year header, (2) weekday row above the month title, (3) Confirm/Apply disabled for a single date, (4) no mobile year selection, (5) past dates unreachable + no rental-vs-search context switch, (6) mobile Confirm must be pinned. Owner decisions (AskUserQuestion): D1 single date → `{from,to:from}`; D2 mobile fixed-header month+year dropdowns; D3 weekday under each month title; D4 pinned Confirm. Fixes routed to **Task 561** (`Sprint_42_kickoff_prompt_Task_561_RangeDatePickerFixes.md`, adds `disablePastDates?` prop). **Next: hand Task 561 to Sonnet.** (Prior session — 2026-07-08 Sprint 42 kickoff refinement — moved to archive.)
...
**Task numbering — last used: 561. Next free: 562.**

**🔵 Sprint 42 (Booking-style RANGE DatePicker) — Task 558 ❌ REJECTED by owner (2026-07-08); fixes = Task 561.** Plan: `tasks/Sprints/Sprint_42_Booking_Range_DatePicker.md`. **Task 558** `RangeDatePicker` component was built by Sonnet but rejected on 6 defects (dup mobile header, weekday above title, single-date not confirmable, no mobile year picker, past dates unreachable + no rental/search context switch, Confirm not pinned). **Task 561** (`Sprint_42_kickoff_prompt_Task_561_RangeDatePickerFixes.md`) routes all 6 back to Sonnet with owner decisions locked (single-day `{from,to:from}`; mobile fixed-header month+year dropdowns; weekday under each month title; pinned Confirm; new `disablePastDates?` prop). **Task 559** listings integration waits for 561 approval (will pass `disablePastDates` per surface). **Task 560** admin suspension-as-range DEFERRED (DB/RLS/server action). 557 kickoff retained as §6t/MantinePopover-props reference. **Next: hand Task 561 to Sonnet.**
