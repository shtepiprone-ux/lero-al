...
**2026-06-15 — Task 243 (Epic BB.2, listing→owner inquiry flow, Path A) — ✅ APPROVED after rework, commit emitted (pending owner run + owner SQL run).** Rework re-reviewed against the real files (not the log): (1) SQL now idempotent — `DROP POLICY IF EXISTS` before each `CREATE POLICY` (session-log block); (2) `email_transient` → partial-success UX (`ListingInquiryDialog.tsx:78-84` closes + resets + `toast.success(inquiry_success_email_pending)`, no "try again"; row already persisted, dup-risk avoided) — owner decision; (3) `DialogContent` `max-w-sm`→`sm:max-w-sm` (`:94`) fixes the <640 full-width bottom sheet, proven by new open-dialog matrix uk@320/375/390 + sq/en/it@375; (4) dual mobile-bar overlap confirmed on-record as owner out-of-scope decision. Controls preserved (verified in `ListingContact.tsx`: Report, WhatsApp/Call, Share, Favorite/Save, guest CTA, owner-deleted/unavailable, closed-disabled all intact); `ListingMobileCTA` untouched; `contact_inquiries` untouched. Gates (per log): tsc=0, vitest 597/597, check:i18n parity, check:i18n-dynamic, build, build-storybook. Log: `docs/sessions/2026-06-15-task-243-listing-inquiry-flow.md`. **Next: owner runs native `git status`/`git diff` to confirm, then the Task 243 commit + the `listing_inquiries` SQL in Supabase.** Minor follow-up: add `listing_inquiries` to `scripts/schema-drift-check.sql`.
...
**Reserved/deferred:** 310 (Epic HH P4), 311 (Epic HH P5 — partially superseded), 313 (Epic HH P6 Verified Agents — blocked on owner DB-schema approval), 316–323 (Epic II P1–3), 238 (Epic Y; ✅ APPROVED, commit emitted pending owner run), 425 (Epic Y; ✅ APPROVED, commit emitted pending owner run — zero-diff finding), 243 (Epic BB; ✅ APPROVED, commit emitted pending owner run + SQL), 246 (Epic DD). **CLOSED:** 351/352/353 (superseded by global DS). Deferred (no task #): **I.3** listing-status helper API migration `(status) → (listing)` — see `docs/domain-rules.md` → "Future ListingStateMachine evolution trigger".
...
| **BB — Listing Inquiries: Report & Message** | **243** ✅ APPROVED (commit emitted, pending owner run + SQL) · **430** KICKOFF READY (drift-guard coverage for `listing_inquiries`) (242 ✅) | BB.2 listing→owner inquiry — Path A shipped: new `listing_inquiries` table (idempotent SQL in session log), `submitListingInquiry` action (server-side owner-email resolve, IP rate-limit, typed errors), `listingInquiry.ts` Resend sender, shared `ListingInquiryDialog` (full-width bottom sheet <640), desktop + mobile triggers. Rework closed SQL idempotency + `email_transient` partial-success UX + `sm:max-w-sm` sheet fix + open-dialog matrix. Log: `docs/sessions/2026-06-15-task-243-listing-inquiry-flow.md`. Kickoff: [`Epic_BB_kickoff_prompt_Task_243_ListingInquiryFlow.md`](../tasks/Epics/Epic_BB_kickoff_prompt_Task_243_ListingInquiryFlow.md). | [`Epic_BB_…`](../tasks/Epics/Epic_BB_Listing_Inquiries_Report_and_Message.md) |
