/**
 * /api/cron/inactivity auth boundary — Task 851 (Sprint 78).
 *
...
function makeRequest(method: 'GET' | 'POST', authHeader?: string): NextRequest {
  ...
  return new NextRequest('http://localhost/api/cron/inactivity', { method, headers })
}

describe.each([
  ['GET', GET],
  ['POST', POST],
] as const)('inactivity %s auth', (method, handler) => {
