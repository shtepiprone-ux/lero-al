...
**Opened:** 2026-09-23 · **Status:** 🟠 **OPEN** · **Landed tasks:** 2 (870, 881) · **Kickoffs filed:** 4 (870, 871, 881, 883) · **Reserved:** 0
...
| **883** | The bell issues one anonymous `notifications` read on sign-out: `useNotifications.ts` calls `fetchAll()` before its `userId` guard, which since Task 881 returns **401** and logs `[notifications] fetch failed` | P3 | Q4 | 881 | `KICKOFF FILED` 2026-09-25 (reserved the same day by 881's review, finding F3; owner-observed at O80-5 step 10). Hosted here by discovery, not goal fit; the owner may move it. Guard in `fetchAll`, clear on sign-out, drop stale responses; owner check **O80-6** → [`…Task_883…`](Sprint_80_kickoff_prompt_Task_883_Notification_Bell_Never_Reads_Without_A_User.md) |
