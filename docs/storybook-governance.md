...
- **Layout**: no story-bearing members remain (FilterBar, PageHeader, PageShell, Section deleted —
  ...
  have no stories)
- **Shared**: Combobox
- **Primitives**: Badge, Button, Checkbox, Command, Dialog, DropdownMenu, Input, PasswordInput, Popover, Select, Sheet, Skeleton, Tabs (`PasswordRequirementsHint` relocated out of `src/components/ui/` to `src/design-system/mantine/patterns/` — Task 873)
...
**Status as landed (Task 812, Revision 1).** The detector is built and proven correct: a two-armed plant (temporarily de-enrolling `CollectionsSection.tsx`) fails and names the exact edge, and restoring the manifest (`git hash-object` identical, `git status --porcelain` clean) clears that specific finding. The raw measured frontier across the 38 manifest roots was 54 `tier1-unenrolled` + 3 `tier2-legacy-primitive` edges. Owner decision 1 (2026-09-11) classified the 11-path/23-edge shared `design-system/mantine/patterns/*` cluster as tier 3, owned by Task 816, and allowlisted it; 27 `tier1-unenrolled` + 3 `tier2-legacy-primitive` edges (26 + 3 = 29 distinct edges — the printed 27 tier-1 count carries one duplicate binding) remained, unowned by Task 812. Owner decision 3 (2026-09-11) first wired `npm run check:rendered-scope` into the `governance` CI job with step-level `continue-on-error: true` — advisory only, visible but non-blocking.

**Blocking, baselined (Task 818, 2026-09-11).** `scripts/rendered-scope-baseline.json` is a versioned (`version` integer), edge-keyed (`"<from> -> <to>"`) JSON object recording the frontier as measured: each entry carries at least its `tier`. The gate now consults it *after* classification — an edge present in the baseline is **baselined debt** and does not fail; an edge **absent** from it is **new** and fails, naming the edge and its tier; a baseline entry with **no matching measured edge** is **stale** and fails, printing the `--update-baseline` command that fixes it. This is a strict ratchet, not a new exemption path: `npm run check:rendered-scope:update-baseline` regenerates the file from a live run (never hand-edited) and **refuses** to record a **new** `tier2-legacy-primitive` edge that was not already in the prior baseline — it writes every other change, prints the refused edge with 16d tier 2's correction ("stop importing the legacy primitive"), and exits non-zero. The three pre-existing tier-2 edges (`ListingCard`/`PopularLocationsView` → `AppImage`, `AuthSheet` → `PasswordRequirementsHint`) are recorded at their measured state as debt that can only shrink, never grow. (`AuthSheet` → `PasswordRequirementsHint` paid off by Task 873 — the hint moved out of `src/components/ui/` and the edge was removed from the baseline.) A CI-safe self-test, `npm run check:rendered-scope:verify` (added to the `governance` job immediately after the gate step, matching the house `*:verify` pattern at `check:hydration:verify` / `check:listing-visibility:verify`), plants and asserts four arms on every PR — a new tier-1 edge, a new tier-2 edge (and its `--update-baseline` refusal), a stale baseline entry, and the clean unplanted tree — so the gate's ability to fail is re-proven rather than trusted from a transcript. `.github/workflows/governance-pr.yml`'s `check:rendered-scope` step lost `continue-on-error` and its advisory name; the class this baseline deliberately does not catch is unchanged from §15.5's own detector limitation — dynamic `import()` and `React.lazy()` are never statically resolved, baseline or not. See `docs/sessions/evidence/task812/` for the Task 812 measured census, `docs/sessions/evidence/task818/` for the baseline generation and planted-arm evidence, and `docs/golden-rules.md`'s GR-1/GR-3 rows for the current enforcement state.

### §15.6 Per-surface GR-1 census command: `check:surface-census` (Task 817, 2026-09-11)

**Why.** GR-1's `Command` block (`docs/golden-rules.md`) has named `scripts/check-surface-census.mjs --surface <path>` since the rule was written, but the script did not exist — GR-1 has been receipt-only by hand since day one. `check:rendered-scope` (§15.5) cannot fill that gap by design: it walks only manifest **roots**, so an **unenrolled** surface is never a root and nothing it renders is ever examined. That is the exact pre-enrolment state Task 809 was in when `/favorites` shipped unmigrated.

**Mechanism.** `scripts/check-surface-census.mjs --surface <repo-relative-or-absolute path>` starts at the named surface — enrolled or not — as node #1, and walks **transitively** through tier-1 nodes only, using the same resolver (`scripts/lib/import-resolver.mjs`), the same canonical-story criterion (`scripts/lib/mantine-story-scope.mjs`), and duplicated (not extracted) copies of `check-rendered-scope.mjs`'s JSX-tag-root extraction, single-hop barrel unwrap, and `src/components/ui/` tier-2 prefix rule — those two files are out of scope for this task and are never edited by it. Every node — a `.ts`/`.tsx` production file, resolved the same way `scripts/lib/import-resolver.mjs` probes (exact path, `.tsx`, `.ts`, `/index.tsx`, `/index.ts` — never `.js`) — is classified into exactly one of `agent-contract` 16d's three tiers (never recursing into a tier-2 or tier-3 node) and printed with its manifest status, whether a canonical Mantine story imports it **directly or through a single-hop `index.ts(x)` barrel re-export — the same resolution the render-side walk uses** (never merely its parent — GR-3), an AST `className`-attribute count, and a per-node legacy-import count (Revision 1, Task 817: this barrel-aware story resolution and the legacy-import count are both corrections to the original landing — see "Status as landed" below).

```bash
node scripts/check-surface-census.mjs --surface <path>            # gate check
npm run check:surface-census -- --surface <path>                  # same, via package.json
node scripts/check-surface-census.mjs --surface <path> --report   # full node table, always exit 0
```

Exit codes: **0** — every node accounted for, GR-1's receipt line printed. **1** — at least one blocking node; `GR-1 CENSUS BLOCKED` is printed naming every offender, and the string `GR-1 CENSUS COMPLETE` never appears in the same run. **2** — the invocation itself is unusable: `--surface` missing, the path absent, non-`.ts`/`.tsx`, or outside the repo root.

**Deliberately does NOT flag — the false-positive boundary, stated rather than silently absorbed:**

| Class excluded | Mechanism | Owning task |
|---|---|---|
| A tier-3 node — a shared component owned by another surface | `scripts/rendered-scope-allowlist.json` entry with a non-empty `reason` **and** `owner`, on a path that is not itself under `src/components/ui/*` | the `owner` recorded in that entry (currently 813, 816) |
| A hook, util, constant, context object, or theme value | Never counted as a node — the census only follows a local import whose binding is used as a JSX opening/self-closing tag root | n/a — not a rendered component |
| A type-only import, or the type-only member of a mixed import | Excluded via the same `isTypeOnly`/`el.isTypeOnly` check `check-rendered-scope.mjs` uses | n/a |
| A component rendered only from a `.stories.tsx` file | Not a production render path; the census only follows imports from production `.ts(x)` files reached transitively from the named surface | n/a — stated in the printed blind-spot sentence |
| Dynamic `import()` / `React.lazy()` | Not statically resolved — same limitation `check-rendered-scope.mjs` already states | n/a — stated in the printed blind-spot sentence, Task 818 territory if it ever needs closing |
| A barrel re-export the single-hop unwrapper cannot resolve | Reported at the barrel file itself, counted separately, never silently treated as covered | n/a |
| A tier-2 or tier-3 node's own further imports | The walk does not recurse past a tier-2 or tier-3 node — 16d's census is transitive, its scope is not | whichever task owns migrating/allowlisting that node |

**Status as landed (Task 817).** Proven against `src/modules/listings/components/FavoritesShell.tsx`: the clean-tree census finds 15 nodes and blocks on exactly the pre-existing, unowned `ListingCard.tsx -> src/components/ui/AppImage.tsx` tier-2 edge (Task 813/818 territory, not this task's to fix). A two-armed plant temporarily de-enrolling both `FavoritesShell.tsx` and `CollectionsSection.tsx` from `scripts/mantine-migration-scope.json` reproduces the pre-Task-809 topology: the census still finds and blocks on `CollectionsSection.tsx` (`manifest:no`) while, in the same planted state, `npm run check:rendered-scope:report` names it **zero** times and `npm run check:story-coverage` still exits 0 with 0 unproven — the exact blind spot decision 2 describes, measured rather than asserted. The manifest and allowlist were restored afterward (`git hash-object` identical, `git status --porcelain` clean). **Landing this command does not itself make GR-1 or GR-3 enforced or blocking** — it is run and its receipt read by hand at task design/execution/review time, same as before. Task 818 (reducing or baselining the remaining frontier) still gates turning anything here blocking in CI.

### §15.7 Diff-mapped per-surface census, blocking: `check:surface-census:changed` (Task 819, 2026-09-11)

**Why.** `check:rendered-scope` (§15.5, blocking since Task 818) only walks **enrolled** roots — a wholly unenrolled surface is never a root and nothing it renders is ever examined. `check-surface-census.mjs` (§15.6, Task 817) closes that gap in principle, but it takes a `--surface` argument, and no CI job knew which surface a given PR was about — so a change to a surface in the exact pre-enrolment state `/favorites` was in before Task 809 was still invisible to CI. Owner decision 4 (2026-09-11) made this blocking, conditional on Task 818's approval, with one hard requirement: **fail closed** — a diff-mapping gate that cannot say what to census must never silently skip or pass with a warning.

**Mechanism.** Two new scripts, one CI-facing entry point:

1. `scripts/map-changed-surfaces.mjs` computes `git merge-base <base> <head>`, then `git diff --name-status` against it, and classifies every changed path: outside `src/`, not `.ts(x)`, a story/test file, or deleted are **excluded** with a stated reason; everything else is a **candidate**. Each candidate resolves **upward** to the surface(s) that render it — not merely to itself — via a breadth-first climb over a whole-`src/` reverse render graph (renderer → rendered JSX-tag edges, built fresh every run, the same binding/barrel-unwrap semantics `check-rendered-scope.mjs` uses, duplicated rather than imported per that file's own out-of-scope convention). A node with no renderer at all, a `scripts/mantine-migration-scope.json` entry, or a route file under `src/app/**` is itself a surface root and is never climbed past. A candidate trapped in a cycle where **no** node in it is ever a root is **unresolved** — the mapper genuinely cannot say what to census, and that is a fail-closed condition, not a skip.
2. `scripts/check-surface-census.mjs` gained an additive `--json` flag (Task 819 R2): one structured object — scope counters, the full node table, the blocking list with reason codes — on stdout, same exit code as the human run for the same surface. Its ordinary human output and `--report` mode are unchanged.
3. `scripts/check-surface-census-changed.mjs` is the runner: it calls the mapper, censuses every included surface with `--json`, and aggregates every blocking node against its own versioned baseline, `scripts/surface-census-baseline.json` — `version` plus a `blocks` object keyed `"<surface> :: <node> :: <reasonCode>"` (one entry per surface/node pair, because the same widely-rendered node can block many different surfaces independently), each value carrying at least its `reasonCode`. **The comparison is diff-scoped, not whole-tree (Task 819 Revision 1) — this is the one place this gate cannot copy `check-rendered-scope.mjs`'s comparator verbatim**, because that gate's walk measures every edge on every run, while this run only ever censuses the surfaces the current diff maps to. Three outcomes, not two: a baselined block on a censused surface is recorded debt and does not fail; a block absent from the baseline (on a censused surface) is **new** and fails, naming surface/node/reason; a baseline entry whose own surface **was censused this run** with no matching block is **stale** and fails, printing the update command — real, measured paid-off debt. A baseline entry whose surface this run did **not** census is **carried**: neither stale nor new, counted separately in the scope block, and never treated as evidence either way. `--update-baseline` **merges** rather than replaces: every prior entry for an un-censused surface survives byte-identically, and only the entries for surfaces this run actually censused are regenerated (so a new `tier2-legacy-primitive` block is still refused, and paid-off debt on a censused surface still drops) — a diff-scoped run must never be able to delete recorded debt for a surface it did not look at. `--update-baseline` only ever updates an **existing** baseline file — a missing file is refused, naming the path and saying to restore it from version control, never silently regenerated. The one-time initial write used a separate, deliberately not-npm-wired `--seed-baseline` flag, exactly once, seeded from the full **candidate space** (every manifest entry, every `src/app/**` route, and every file nothing in the whole-src render graph renders) rather than from one diff's mapped surfaces — that produces the actual debt state at every diffable surface, not just the surfaces one arbitrary PR happened to touch.

```bash
npm run check:surface-census:changed                    # gate check (CI: base/head from SURFACE_CENSUS_*_SHA env)
npm run check:surface-census:changed:update-baseline     # regenerate an EXISTING baseline from a live run
npm run check:surface-census:changed:verify              # CI-safe, 6-arm gate self-test
```

**Fail-closed conditions (decision 4, verbatim: "never a skipped check and never a pass with a warning"):** the merge base cannot be determined; the changed-file count or the mapped-surface count exceeds a stated, configurable limit (`--max-changed-files`/`--max-surfaces`, printed on every run); a changed `.ts(x)` under `src/` resolves to no surface (unresolved candidate); a mapped surface's own census exits 2 (its invocation was unusable); the baseline is missing, unparseable, or version-mismatched. Every one of these is a failing run, named on its own line, never a silent skip.

**Cannot see, stated on every run:** dynamic `import()` and `React.lazy()` (not statically resolved — the same limitation `check-rendered-scope.mjs` states); a surface reached only through a route convention the mapper does not model; a file changed outside `src/`; a component rendered only from a `.stories.tsx` file (`check-surface-census.mjs`'s own pre-existing blind spot, unchanged by this task). The mapper's whole-`src/` render graph, and therefore this gate, also depends on the workflow checkout's `fetch-depth: 0` (already set, `.github/workflows/governance-pr.yml`) to compute a merge base at all — a future PR that shallows the checkout would make this gate fail closed rather than silently stop working, which is the correct failure direction but is worth knowing before it happens.

**Status as landed (Task 819).** Measured across four real historical base/head pairs spanning Tasks 803–810 (587 combined changed files): **zero** unresolved candidates — the mapper's fail-closed strictness is livable on this tree. The seed baseline recorded 140 pre-existing blocking (surface, node) pairs from a live run at Task 819's execution commit. See `docs/sessions/evidence/task819/` for the full measured census, the planted-arm proofs, and `docs/golden-rules.md`'s GR-1/GR-3 rows for the current enforcement state.

### §15.8 One rule for the pattern directory: `check:pattern-enrolment` (Task 820, 2026-09-11)

**Why.** §15.5–§15.7 all walk from the manifest or from a diff outward — none of them asks the direct question "is every `.tsx` file under `src/design-system/mantine/patterns/` actually enrolled." Task 816 measured that split as an artefact of history, not a designed governance model: 5 enrolled, 11 tier-3-allowlisted (three of which measurably failed their own "shared" premise), 17 governed by neither mechanism. Owner decision 5 (2026-09-11, `docs/design-system-pattern-ownership.md` §4) picked "one rule for the whole directory": every current and future `.tsx` file there is an enrolled pattern, full stop, and the eleven tier-3 allowlist entries are retired as no longer needed. Task 820 both did that one-time enrolment (`scripts/mantine-migration-scope.json` 38 → 66; `scripts/rendered-scope-allowlist.json` 13 → 2, leaving only the two entries owned outside the design system) and added the gate that keeps it true for file 34.

**Mechanism.** `scripts/check-pattern-enrolment.mjs` reads the directory listing (`readdirSync`, top-level `*.tsx` only — never a hard-coded name list, which would silently stop enforcing the rule the moment a new pattern file is added) and the manifest, and fails in either direction: a `.tsx` in the directory absent from the manifest is **unenrolled**; a manifest entry under that directory prefix whose file no longer exists is a **ghost entry**. Both conditions are checked by one pure function (`evaluatePatternEnrolment`) shared by the real run and the `--verify-gate` self-test, and the exit-code decision is its own pure function (`evaluateGateExitCode`) for the same reason `check-rendered-scope.mjs`/`check-surface-census-changed.mjs` split theirs out (Task 818 R15 precedent). Deliberately a new, small script rather than an arm inside `audit-design-system-patterns.mjs` (Task 816) — that command verifies whether the (now-empty) tier-3 allowlist's own premise still holds, a different question, and R8 of Task 820 leaves it unmodified so a CI-blocking gate never shares an exit code with an audit the owner deliberately left unwired.

```bash
npm run check:pattern-enrolment          # gate check (CI default, blocking in the governance job)
npm run check:pattern-enrolment:verify   # CI-safe, 5-arm gate self-test
```

**Cannot see, stated on every run:** `index.ts` (the barrel — not `.tsx`), any `__tests__/` subdirectory, `*.module.css`, and any `.tsx` file this exact directory does not itself contain — a pattern added anywhere else is invisible to this check by construction and remains `check:rendered-scope`'s/`check:story-coverage`'s concern, not this one's; a pattern file this script cannot read (permissions/symlink) is skipped, never silently treated as compliant.

**A pre-existing gap this task also closed, discovered while enrolling the 28.** `check-story-coverage.mjs` resolves an import specifier to a concrete file path but never unwraps a barrel re-export — it is `check-rendered-scope.mjs`/`audit-design-system-patterns.mjs` that do the single-hop `index.ts(x)` unwrap, not this gate. Twenty-seven of the twenty-eight newly-enrolled patterns' own canonical Stories imported their component through the shared `@/design-system/mantine/patterns` barrel, which resolves to `index.ts` itself — never to the individual component path the manifest names — so all twenty-seven read as enrolled-but-unproven the moment they were enrolled, contradicting this task's own kickoff §3.1 ("will accept the other 27 the moment they are enrolled"), which assumed the resolver unwraps barrels. Measured directly (`docs/sessions/evidence/task820/`) before relying on the assumption. Fixed by switching each of the 27 story files' own import to the direct component path — the same convention `MantineAddItemPanel.stories.tsx`/`MantineFilterSection.stories.tsx` already documented for the five originally-enrolled patterns — a one-line, zero-behavior-change edit per file, not a design decision.

**Status as landed (Task 820).** `scripts/mantine-migration-scope.json`: 38 → 66 (28 added — all remaining pattern-directory files); `scripts/rendered-scope-allowlist.json`: 13 → 2 (the eleven Task 816 tier-3 entries retired); `scripts/rendered-scope-baseline.json`: 29 → 30 edges (one new tier-1 edge, `MantineListingGalleryPattern.tsx -> LightboxView.tsx`, introduced by enrolling `MantineListingGalleryPattern` as a root; zero new tier-2 edges); `scripts/surface-census-baseline.json`: 690 → 690 (this task's own diff touches no production `src/` consumer file — only the manifest/allowlist/messages/story files — so the diff-scoped `check:surface-census:changed:update-baseline` correctly carries every entry forward rather than dropping any; the specific already-baselined blocks referencing now-enrolled-and-storied patterns, e.g. `MantinePopover`/`responsiveBottomSheet` on `src/app/[locale]/layout.tsx`, become stale only once a future PR's diff actually touches that surface). `npm run audit:design-system-patterns` exits 0 (33 enrolled / 0 tier-3 / 0 ungoverned) because the invalid tier-3 premise no longer exists, not because the audit changed. See `docs/sessions/evidence/task820/` for the full measured census, the R3 pre-change frontier projection, and the planted-arm proof.

### §15.9 A second directory, the same rule: `check:media-enrolment` (Task 813, 2026-09-11)

**Why.** §15.8's directory rule is scoped to `src/design-system/mantine/patterns/`. Owner decision §5.1 (C) (2026-09-11) needed a home for `AppImage.tsx` — the project's canonical, non-Mantine `<img>` render site (Cloudinary srcset, LQIP, React 19 `preload`, `imageGuard`/predictive preloading) — outside `src/components/ui/`, whose path prefix alone both `check-rendered-scope.mjs` and `check-surface-census.mjs` classify as `tier2-legacy-primitive` (`agent-contract` 16d tier 2: "the fix is 'stop importing it', not 'enrol the file'"). The decision refused the Mantine-pattern directory by name — a bespoke `<img>` renderer is not a Mantine pattern — and opened a sibling directory, `src/design-system/media/`, instead. The decision's own words: the new directory "does not ship ungoverned."

**Mechanism.** `scripts/check-media-enrolment.mjs` is `check-pattern-enrolment.mjs`'s shape applied to the new directory — a `readdirSync` top-level `*.tsx` listing (never a hard-coded name list), a manifest cross-check failing in both directions (**unenrolled**: a `.tsx` present, absent from the manifest; **ghost entry**: a manifest entry under the prefix whose file no longer exists), one pure `evaluateMediaEnrolment` classifier and one pure `evaluateGateExitCode` shared by the real run and its `--verify-gate` self-test. Deliberately its own script, not an arm added to `check-pattern-enrolment.mjs` or `check-media-enrolment.mjs` importing from it — two directories, two independent checks (R12/1a of the kickoff). Canonical-Story coverage stays `check:story-coverage`'s job; this check states that explicitly on every run so a green `check:media-enrolment` is never mistaken for Story proof.

```bash
npm run check:media-enrolment          # gate check (CI default, blocking in the governance job)
npm run check:media-enrolment:verify   # CI-safe, 5-arm gate self-test
```

**Cannot see, stated on every run:** `*.module.css` and non-`.tsx` siblings (`appImageConfig.ts`, `useAdaptiveImageConfig.ts` — hooks/config, not components), any `__tests__/` subdirectory, and any `.tsx` this exact directory does not itself contain.

**Status as landed (Task 813).** `AppImage.tsx` and its co-located siblings (`appImageConfig.ts`, `useAdaptiveImageConfig.ts`, `AppImage.module.css`) moved from `src/components/ui/` to `src/design-system/media/` byte-identically (a pure rename — zero content changes in any of the four files; every one of their own imports was already relative or otherwise path-independent). `scripts/mantine-migration-scope.json`: 66 → 67 (`src/design-system/media/AppImage.tsx` added); a new canonical Story, `Mantine/Primitives/AppImage`, gives it Story coverage. `scripts/rendered-scope-baseline.json`: 30 → 28 edges (the two `AppImage` tier-2 edges removed; zero new edges, since AppImage is now enrolled-to-enrolled). `scripts/surface-census-baseline.json`: 690 → 676 (15 stale entries dropped — 13 `AppImage` tier-2 blocks whose diff-mapped surfaces this task's importer rewrites happened to touch, plus 2 pre-existing `MantinePopover`/`responsiveBottomSheet` blocks on `src/app/[locale]/layout.tsx` that Task 820 had already paid off but no prior diff had touched; 1 new entry added — `MantineListingGalleryPattern.tsx :: LightboxView.tsx :: tier1-unenrolled-or-unstoried`, pre-existing debt already recorded under two other surface keys, newly visible for this specific pairing). `LightboxView.tsx`'s own unenrolled status is unchanged and out of this task's scope (owner decision §17.6 on Task 820 forbade enrolling it merely to hide the tier-2 hop); `check:surface-census --surface` on both `MantineListingGalleryPattern.tsx` and `MantineListingDetailPattern.tsx` reports zero `tier2-legacy-primitive` blocks. See `docs/sessions/evidence/task813/` for the full census, baseline diffs, and the planted-arm proof.

### §15.10 Card-track monotonicity gate: `check:card-track-monotonicity` (Task 815, 2026-09-16)

**Why.** `measureStorybookColumnMonotonicity` (`scripts/task809-favorites-parity-probe.mjs:319-374`) checks one Story (`Mantine/Primitives/FavoritesShell → Populated`), widths `600-900` only, targets the first element on the page with `display: grid` (not the track itself), and runs only from that probe's own `main()`, which first drives `/favorites` and `/listings` on a production server with an authenticated storage state — no `package.json` script and no CI job ever invoked it. The other 15 canonical Stories that render `MantineListingCardTrack`, and every rung above 900px, were unchecked: a padding step removing a column at 1024, 1280, or 1536 would have shipped green.

**Mechanism.** `scripts/check-card-track-monotonicity.mjs` runs against nothing but `storybook-static/` — its own `node:http` static server (its own port, distinct from `check-homepage-grid.mjs`'s 6020 and the 6006 dev server), Playwright Chromium, no `next start`, no storage state. It derives the track's selectors at runtime from the one built CSS asset matching `^MantineListingCardTrack-.*\.css$` (the CSS Module classes are hashed per build — no literal hashed class name is ever written into the script), discovers scope by loading every `storybook-static/index.json` Story whose title satisfies `scripts/lib/mantine-story-scope.mjs`'s `isCanonicalMantineTitle` (imported, never re-implemented) at `320×900` and `1440×900`, and keeps a Story in scope when either load renders at least one track. **Discovery readiness (Route A, adopted 2026-09-16 per Revision 2):** waits for `window.__STORYBOOK_PREVIEW__.currentRender` to reach `id === storyId` and `phase === 'finished' | 'errored'`, rather than a body-class signal — Revision 1's `sb-show-main` signal was proven unreliable at runtime (it resolves while the track is still unmounted in 5 of 6 samples, `docs/sessions/evidence/task815/58b_`) and was replaced only after a mechanical 32-of-32-sample probe (`docs/sessions/evidence/task815/62_probe-render-phase.txt`) confirmed the render-phase signal correlates with the track actually being mounted, at resolve and 2000ms later, on all 16 in-scope Stories at both discovery widths plus two no-track canonical Stories. A canonical Story that never reaches that phase within the timeout, or whose phase is `errored`, is a **named discovery failure** (exit 1) — never silently dropped out of scope. Each in-scope Story is then swept at `320 479 480 639 640 767 768 1023 1024 1279 1280 1439 1440 1535 1536 1920 2560` (height 900, `locale:en`) — every declared Mantine rung (`theme.ts` `xs 20em`/`xs2 30em`/`sm 40em`/`md 48em`/`lg 64em`/`xl 80em`/`xxl 90em`) and the `.container-wide` 1536px step, each sampled at `rung-1, rung`, plus 320/1920/2560 bounds. Each track is measured by document order: grid → `grid-template-columns` track count; rail → number of direct children whose box lies entirely within `[rail.left, rail.left + rail.clientWidth]` at `scrollLeft` 0. A measure that is lower at a wider sampled width than at the immediately preceding narrower one is a failure naming the Story, track index, mode, both widths, both measures, and both track widths in px. No baseline file, no allowlist, no per-Story exception — the gate reads only `index.json`, the one CSS asset, and served static files.

**Scope — owner decision (2026-09-16, quoted verbatim):** "**Виключити System/\*** — Gate перевіряє лише канонічні Mantine/Patterns Stories, виключення друкується на кожному запуску; виправлення обгортки — окрема задача. Слабше: легасі-Stories лишаються сліпою зоною." The 11 legacy `System/*` Stories that also render the track (`FeaturedListings`/`LatestListings`/`RecentlyViewedSection`/`SimilarListings`, four `--default`/`--populated` variants) are excluded from scope entirely — never navigated to — and the exclusion is printed on every run. Measured at task-design time (`docs/sessions/evidence/task815/design/`): those four System/* Stories drop rail fully-visible 5→4 between 1441 and 1920 (localized to exactly 1535→1536px — the track narrows 1280→1216px as the `.container-wide` gutter steps to 48px, on both the Storybook canvas wrapper and the story's own container), a legacy story-harness artefact rather than a production defect (`patterns-mantine-homepagelistinggrids--default`, the production composition, holds 5 cards at every width 1441-2560). Filed as **Task 827** (fix the double `.container-wide`), owned separately from this gate.

**CI placement.** Blocking in the `homepage-grid` job of `.github/workflows/governance-pr.yml`, as two steps between `Build Storybook` and `Homepage grid invariants gate` — load-bearing placement, because `check:homepage-grid` was measured red on `HEAD` at task-design time (248/260 FAIL, still asserting pre-Task-806 `SimpleGrid` column steps on Stories that now render rails) and is owned separately by **Task 828**; placing this gate before that already-failing step is the only way it runs on every PR.

```bash
npm run check:card-track-monotonicity          # gate check (CI default, blocking in the homepage-grid job)
npm run check:card-track-monotonicity:verify   # CI-safe, 5-arm gate self-test
```

**`--verify-gate` (R7), five arms, no repository file written:** **(a)** negative — the real tree, every in-scope Story clean. **(b)** grid plant and **(c)** rail plant both call `sweepStory` — the SAME sweep implementation, and the SAME `evaluateSweep` call, the real run uses — with a plant (`{mode, rung, paddingPx}`) applied on every navigation, over the full 17-width list; there is no private plant-arm comparator. **(b)**: `padding-inline: 80px` at `min-width: 1024px` on the track's parent element (tagged with a `data-task815-plant` attribute by the plant itself, via `page.addStyleTag` after navigation) on `patterns-mantine-listingcardtrack--grid`; the full-bleed track (viewport width) needs ≥872px for 3 columns, so 1024px minus 160px combined padding leaves 864px → 2 columns. **(c)**: `padding-inline: 140px` at `min-width: 1280px` on `patterns-mantine-listingcardtrack--rail`; 1280px minus 280px combined padding leaves 1000px, below the `64em`/1024px container rung → 3 fully visible instead of 4. Each plant arm asserts the produced track width first (a plant that silently did not apply is an arm failure, not a pass), then requires `evaluateSweep` to report **exactly one** failure naming this Story, `track 0 (<mode>)`, the `prevWidth->rung` pair, and a `measure X->Y` with `Y < X` (parsed and checked, not just string-matched), then reloads the Story unplanted and requires a clean full-17-width sweep. **Mutation-proof (Revision 1, `docs/sessions/evidence/task815/57_mutation-probe.mjs`):** disabling `evaluateSweep`'s own drop comparator makes arms (b) and (c) fail — proof the plant arms exercise the gate's real decision path, not a re-derived copy. **(d)** selector fail-closed — calls `runGate` itself with a non-matching asset pattern and a collecting `log`, and passes only when `runGate` returns exit 1 **and** the collected output contains the `cannot see:` line (Revision 2 — replaces a Revision 1 evidence-only CLI flag with a reusable option any caller can exercise). **(e)** scope fail-closed — discovery with a title predicate that matches nothing must find zero in-scope Stories (the real gate would exit 1).

**Cannot see, stated on every run:** a breakpoint not in the width list; locales other than `en`; a track rendered only after interaction; a drop that recovers between two sampled widths not at a declared breakpoint; a Story whose track mounts only after its Storybook render reaches `finished` (Route A's own blind spot).
