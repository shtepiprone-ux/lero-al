...
**2026-07-05 — Task 552 (Sprint 41 / Epic MM Phase-2 Slice 2 · YearCombobox → MantineCombobox) — ✅ DIFF-LEVEL REVIEW PASS (orchestrator), commit emitted, pending owner NATIVE gate + run.** Verified against the real files: (1) primitive gained `inputMode?`/`onInputChange?` — desktop trigger + mobile sheet both threaded; suppression rule (`if (!onInputChange) onChange('')`) + `inputMode` dual-threading present and covered by smoke assertions + confirmed planted-violation transcripts; (2) `portal` kept on `YearCombobox`'s public API as a documented no-op (not destructured/forwarded), rendered-proof-confirmed un-clipped on both filter surfaces; (3) **desktop `Combobox.Options` scroll cap = `mah={220}`+`overflowY:auto`, canonical-first to `MantineSelect` — orchestrator INDEPENDENTLY corroborated 220px as Mantine `<Select>`'s built-in default (`@mantine/core OptionsDropdown.mjs:114` → `maxDropdownHeight ?? 220`), NOT the legacy 224px** ⇒ clause 16a satisfied; (4) `YearCombobox` legacy import gone, `Props` byte-identical, zero hardcode (all i18n), one story-only key `combobox_numeric_placeholder` in all 4 locales, product code zero new keys. **Owner-native items pending (single-writer / clause 14):** `screenshots:assert --mantine-only` 462/480·0 FAIL claim, six-consumer empty-diff, file-integrity — authoritative on the owner's native run. Session: `docs/sessions/2026-07-05-task552-yearcombobox-mantine-migration.md`. **Next: LocationCombobox (larger — admin add-location sub-flow).**
