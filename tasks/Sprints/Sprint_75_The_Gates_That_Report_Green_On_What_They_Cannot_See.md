...
| **825** | `LightboxView` leaves Tailwind, then is enrolled — the last tier-1 node the gallery surface renders | P2 | **Q4** (was Q2 — critical-flow row 612) | **NEEDS REVISION** 2026-09-17 (Opus implementation review 2) — Revision 1 accepted visually by the owner; returned on one defect: wrapping last↔first animates the strip/rail through the whole row. Kickoff R12 ("like any other step") was the defect; R17–R18 / AC15–AC16. Route: kickoff §17. Prior: **NEEDS REVISION** 2026-09-17 (Opus implementation review 1) — R1–R4 migration accepted; the owner returned the visual matrix (mobile rail overflows the screen, native scrollbar under the strip, carousel does not follow the active photo — all pre-existing since 824, measured). Owner decisions 2026-09-17 in kickoff §16.2; 7 stale census rows removed by scoped proof (§16.3), mapper gap → **831**. Route: kickoff §16. Prior: **READY FOR SONNET** 2026-09-16, **after 797's commit** (`theme.ts`). Rescoped by **owner decision 2026-09-16** (quoted below): the reserved "one manifest line" would have marked a component with 13 Tailwind `className`s migrated. Migrates `LightboxView` and the Tailwind position API of `GalleryNavActionIcon`/`GalleryDesktopNavigation`, value-preserving (rect/computed-style probe), enrols it, removes the 8 stale baseline entries by their writers, re-runs the 612 regression set and `check:click-shield`; owner visual matrix. Takes `LightboxView` from **794**. → [`Sprint_75_kickoff_prompt_Task_825_…`](Sprint_75_kickoff_prompt_Task_825_LightboxView_Migration_And_Enrolment.md) |
| **826** | Task 821's Story JSDoc — remove the stale `owner: "821"` allowlist claim and render the cited `ListingDetailView` call site | P3 | Q2 | **RESERVED** 2026-09-16 by Task 821's review (P3 notes). Full text in `docs/backlog.md` registry. Kickoff not yet written. |
| **827** | Legacy `System/*` listing Stories nest `.container-wide` twice — the rail loses a card at 1536px | P2 | Q3 | **RESERVED** 2026-09-16 by Task 815's design census. `.storybook/preview.tsx:138` (`withCanvas`) and each story's own `.container-wide mx-auto px-4 py-8` (e.g. `src/stories/FeaturedListings.stories.tsx:84`) both step to 48px padding at 1536, so `system-{featuredlistings,latestlistings,similarlistings}--default` and `system-recentlyviewedsection--populated` narrow 1280→1216px and drop 5→4 visible cards (`docs/sessions/evidence/task815/design/02_drop-attribution.json`); the production composition (`HomepageListingGrids`, single container) does not. Excluded from 815 by owner decision 2026-09-16. Kickoff not yet written. |
| **828** | `check:homepage-grid` is blocking in CI and red — 248 of 260 cells assert the pre-806 grid | P1 | Q4 | **RESERVED** 2026-09-16 by Task 815's design session. Fresh build on `1ed5cd2a5`: `TOTAL: 12/260 PASS, 248 FAIL`, exit 1 (`docs/sessions/evidence/task815/design/03_check-homepage-grid_HEAD.txt`) — I-A/I-B/I-D still expect `SimpleGrid` column steps and gaps on `System/*` Stories that render `MantineListingCardTrack` rails since Task 806 (`grid-not-found`, `columnGap=normal`); only I-C (header, 12/12) passes. Blocks every later step of the `homepage-grid` job. Kickoff not yet written. |
| **829** | Manifest enrolment is not a Tailwind check — enrolled patterns still render Tailwind utilities while the census reads `manifest:yes` | P2 | Q2 | **RESERVED** 2026-09-16 by Task 825's design census. Heuristic scan of `className` literals across all 72 manifest entries (`docs/sessions/evidence/task825/design/02_manifest-tailwind-utility-scan.txt`): `MantineListingGalleryPattern.tsx` 9, `MantineListingDetailPattern.tsx` 4, `ListingDetailView.tsx` 1; it misses constant-held strings such as `GalleryDesktopNavigation`'s `NAV_VARIANTS`, so it undercounts. Needs a real detector (AST, constant-aware) with a stated boundary, then the migrations it finds. Kickoff not yet written. |
...
| **831** | `check:surface-census:changed:update-baseline` cannot retire a stale parent row when the child is enrolled in the same diff | P2 | Q4 | **RESERVED** 2026-09-17 by Task 825's review. The mapper stops climbing at a freshly-enrolled manifest root, so the 7 `… :: LightboxView.tsx :: tier1-unenrolled-or-unstoried` rows (parents `ListingGallery`, `ListingDetailView`, `MantineListingDetailPattern`, `MantineListingGalleryPattern`, 3 route pages) were never censused and survived the writer (`docs/sessions/evidence/task825/15_surface-census-changed_update-baseline.txt`). A stale row fails the next unrelated PR that touches a parent. Fix the writer to census every baseline parent whose key names a changed file; two-armed plant. Kickoff not yet written. |
| **830** | `raw-dimension-responsive-prop` stops scanning a file silently on an unbalanced object, and §23.1.d omits two blind spots | P3 | Q4 | **RESERVED** 2026-09-16 by Task 797's review. `findResponsiveDimensionFindings`'s `if (matchEnd === -1) break;` ends the file's scan with no finding when a `prop={{` opener never balances (a `'{'` inside a string value, or `prop={{` text inside a string) — every later site in that file is invisible; measured latent (0 of 651 `src` files). Report it as `<prop>: unparsed-object` and resume after the opener, two-armed test. Also state in §23.1.d: template-literal values (`` `12px` ``) and a line carrying `//` inside a string (truncated before scanning, shared with the single-brace arms). Kickoff not yet written. |
...
822/797 share the design-tokens detector) · **743** independent. **826 · 827 · 828 · 829 · 830 · 831** have no kickoff yet (830 edits the same detector as 797 — after 797's commit; 831 edits the census writer 825 §16.3 works around — after 825's commit). **825** is back in execution on kickoff §17 (2026-09-17, review 2).
