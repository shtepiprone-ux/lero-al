...
**2026-07-02 — Task 524+526 (`MantineTooltip`, P1.22) ✅ APPROVED (code-verified on diff + owner rendered matrix), Tooltip commit emitted — pending owner native gate. Batch C (P1.18–P1.22) COMPLETE.** Canonical `MantineTooltip`: hover/focus Mantine `Tooltip` §6k chrome ≥640 · tap→`ResponsiveBottomSheet` <640. 526 fixed both owner rejections: desktop long-label clip → `multiline`+`maw="16.25rem"` (260px) wrap (rejected it@680 verified wrapping on-screen); story now shows all 4 placements (top/right/bottom/left). §6k chrome exact (bg `#1d2939`/white/12px·500/radius lg/8px·14px/shadow-md cited/withArrow/`events{hover,focus}`); `DragHandle`=1; foundation untouched. tooltip_* keys ×4 (→2049). §6k+§25 carry the documented wrap-divergence. Logs: `2026-07-02-task524-tooltip-bottom-sheet.md` + `2026-07-02-task526-tooltip-wrap-no-clip.md`. Prior: Task 523 (Drawer) `04cc304db`; clause 16 + Sprint 40 (Task 525 audit) still pending owner run. **Next: Sprint 40 Task 525 (TailAdmin conformance audit of ALL primitives). Next free: 527.**
