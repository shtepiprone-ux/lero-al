...
| **815** | The column-monotonicity check runs for one story at one breakpoint band | P1 | **Q4** (was Q2 — a new blocking gate needs planted proof) | **NEEDS REVISION** 2026-09-16 (Rev 2, kickoff §17 — Rev 1 stopped correctly: `sb-show-main` races the render; **owner decision 2026-09-16**: Storybook render phase if a 32/32 probe holds, else a deterministic 15 s wait). `check:card-track-monotonicity`: runtime discovery of every canonical Story rendering `MantineListingCardTrack` (16 measured), 17-width rung sweep, grid columns / fully visible rail cards must not drop as width grows, 5-arm `--verify-gate`, blocking in the `homepage-grid` job ahead of its red step. Scope by **owner decision 2026-09-16** (quoted below): canonical Stories only, exclusion printed every run. Measured on `HEAD`: 16/16 canonical clean; 4 `System/*` Stories drop 5→4 at 1535→1536 → **827**; `check:homepage-grid` red 248/260 → **828**. → [`Sprint_75_kickoff_prompt_Task_815_…`](Sprint_75_kickoff_prompt_Task_815_Card_Track_Monotonicity_Gate.md) |
