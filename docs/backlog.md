...
**2026-06-15 — Task 428 (mojibake/UTF-8 encoding guard) — 🔁 ROUTED BACK after orchestrator review (NOT committed).** Gate scaffolding is sound (script/allowlist/CI step/docs all verified present), but review found 2 confirmed defects: (1) signature blind spot — `SIGNATURES` omits the Albanian ë/ç mojibake pair (U+00C3 + U+00AB / U+00A7), the C-circumflex (U+00C2) family, and the Cyrillic D/N families, so a planted Albanian-accent artifact passes the gate; (2) unjustified `docs/backlog.md` allowlist entry — native run proved the tree stays clean without it. Both native-confirmed (owner PowerShell: clean baseline exit 0; allowlist-removal exit 0). Rework opened as **Task 429** (`tasks/kickoff_prompt_Task_429_mojibake_signature_coverage_and_allowlist_fix.md`). **Next: Sonnet executes 429; Task 426 still pending owner-run commit.**
...
**Task 428 (governance/tooling, 🔁 ROUTED BACK — superseded by Task 429):** `check:mojibake` gate scans tracked text files for UTF-8-as-CP125x double-encoding artifacts and U+FFFD, blocking in CI (`governance-pr.yml`), with `scripts/mojibake-allowlist.json`. Scaffolding verified present (script/script-entry `package.json:53`/CI step `governance-pr.yml:68`/docs note). Routed back at review for 2 confirmed defects (see Task 429). Kickoff: `tasks/kickoff_prompt_Task_428_mojibake_encoding_guard.md`. Log: `docs/sessions/2026-06-15-task428-mojibake-encoding-guard.md`.

**Task 429 (governance/tooling, OPEN — rework of 428):** extend `scripts/check-mojibake.mjs` `SIGNATURES` with the Albanian ë/ç mojibake pair, a specific C-circumflex (U+00C2) set (NBSP/guillemets/copyright/registered/degree — not the bare char), and paired Cyrillic D/N sequences (not the bare chars) validated for 0 false positives against `messages/uk.json`; remove the unjustified `docs/backlog.md` entry from `scripts/mojibake-allowlist.json` (other 4 entries kept); complete integrity transcript (428 covered 7/9 touched files). Origin: Task 428 review (2026-06-15), both defects native-confirmed. Kickoff: `tasks/kickoff_prompt_Task_429_mojibake_signature_coverage_and_allowlist_fix.md`.
