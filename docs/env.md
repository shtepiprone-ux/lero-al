...
CONTACT_SUPPORT_EMAIL=support@lero.al # Recipient for general/support/other inquiries (server-only). Also used as fallback when CONTACT_SALES_EMAIL is absent.
CONTACT_SALES_EMAIL=sales@lero.al     # Recipient for sales/partnership/press inquiries (server-only). Falls back to CONTACT_SUPPORT_EMAIL if unset.
...
CRON_SECRET=your_cron_secret               # Random secret (e.g. openssl rand -hex 32). Set in Vercel → Settings → Environment Variables. Vercel automatically passes it as Authorization: Bearer <CRON_SECRET> to cron routes defined in vercel.json. Read by every cron route via src/lib/cron/verifyCronRequest.ts: /api/cron/inactivity, /api/cron/listings-expiry, /api/cron/price-alerts, /api/cron/saved-searches and /api/cron/listing-activity (Task 849). Unset or empty = every cron call is refused (401) — set it in Production before deploying Task 851 or manual POST triggers are blocked too (Task 851).
