...
**Task numbering.** Last used: **401** (Epic JJ Task 401 token-foundation kickoff — READY, not yet executed). 400 = file-integrity gate — COMPLETE, pending commit. 399 = harden i18n scanner vs `{'...'}` evasion — COMPLETE, pending commit. 398 = story-coverage gate + scaffold — **APPROVED, commit emitted**. 397 = COMMITTED. 396 = COMMITTED. 394+395 = COMMITTED. **Active focus: orchestrator review + commit 400 + 399 (398 done); then Epic JJ Task 401 (Design Variables foundation).** Next free: **402**. Reserved/deferred: 310 (Epic HH P4), 311 (Epic HH P5 — partially superseded), 313 (Epic HH P6 Verified Agents — blocked on owner DB-schema approval), 316–323 (Epic II P1–3). CLOSED: 351/352/353 (DS-6/7/8 — superseded by global DS). Deferred (no task #): **I.3** listing-status helper API migration `(status) → (listing)` — see `docs/domain-rules.md` → "Future ListingStateMachine evolution trigger".
