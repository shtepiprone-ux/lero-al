...
**2026-06-15 — Task 427 (Epic I, admin/owner full edit + any-status access) — ✅ APPROVED, commit emitted (pending owner run).** Rework re-reviewed against real code (not the log): R1 `applyListingTransition.ts:231–234` same-status no-op removed → `invalid_transition` (admin+owner tests assert it `applyListingTransition.test.ts:214–226`; non-owner still `forbidden`); R2 delete confirmed status-gate-free (RLS ownership-only) + proof trace; R3 `npm run build` GREEN (39/39, exit 0); R4 rendered matrix 6 cells PASS (uk@320/375/390, 5-button ListingPreviewDialog + 6-option StatusChangeControl). Single-write invariant + base matrix + semantic helpers unchanged. Non-blocking cleanups (stale "no-op" wording in Negative-flow row; duplicated inherited-proof section in log; backlog edit was not listed in the rework Files Changed table) — fix on next touch. Log: `docs/sessions/2026-06-15-task427-admin-owner-full-edit-and-status-access.md`. **Next: owner runs the Task 427 commit.**
