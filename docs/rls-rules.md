...
| `0024_permissive_rls_policy` (INSERT) | `public.listing_views` ("Anyone can insert a view") | Anonymous page-view tracking is intentional. Guards live in `record_listing_view` RPC (rate limit, IP hash). DDL has rationale comment. ⚠️ **Rationale falsified 2026-09-21 (Task 850 closure → Task 865): the cited guards do not apply to this policy.** Measured: the policy is `PERMISSIVE`, `roles {public}`, `with_check = true`, and `anon` holds a direct `INSERT` grant — so an anonymous caller can insert `listing_views` rows straight through PostgREST, **bypassing `record_listing_view` and every guard this row relies on**. It is the only policy in the whole schema whose predicate does not depend on `auth.uid()`. Nothing in `src/` writes this table directly (the tracker calls the `SECURITY DEFINER` RPC, which bypasses RLS anyway), so dropping the policy does not break view tracking. Inflated rows feed 849's `listing_activity_daily.recorded_views` and the Sprint 78 dashboards. **Do not cite this exception as guarded until 865 resolves it.** | Task 269 (rationale comment) · falsified by Task 865 |
