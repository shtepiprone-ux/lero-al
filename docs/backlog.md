...
**Designed — homepage → 100% Mantine (audit 2026-07-20; plan A→C→B, D deferred; blocks A/646 + C/647 ✅ done → archive):**
- **Task 648 (block B slice 1/2)** — created canonical `PopularLocationsView.tsx` + `Mantine/Primitives/PopularLocationsView` story (Title/SimpleGrid/Box + kept AppImage/CITY_GRADIENTS/hover-focus classes); does not touch `PopularLocations.tsx` (untouched, verified). typecheck/check:stories/check:story-coverage/check:i18n/mojibake all 0; Q3 rendered proof (7 captures: mobile/tablet/desktop × 4 locale samples + long-name truncate stress, both image+fallback branches). `mantine-migration-scope.json` deliberately not extended (unbroken prior-task precedent — P3 policy note, non-blocking). ✅ APPROVED WITH NOTES (orchestrator review 2026-07-20) — component/story correct, provenance full; gates all green: typecheck 0 / mojibake 0/1840 / **`npm run build` ✓ (75s, 40/40 pages)** (owner-native) + check:stories 122/0 + check:story-coverage + check:i18n 2206×4 (reviewer-reproduced). `bdrs="xl"` + fixture-URL self-catches sound. **Story-first slice 1/2; Task 649 wires it into `PopularLocations`.** Session: `docs/sessions/2026-07-20-task648-popularlocationsview-mantine-component-story.md`. Committed `cf1835399` (governance build-gate `67340ff49` committed separately).
- **Task 649 (block B slice 2/2)** — refactor `PopularLocations.tsx` (keep Supabase query/localization/href + null-guard) to map rows → `PopularLocationsViewLocation[]` and render `<PopularLocationsView/>` (Task 648); remove inline `<section>`/`<h2>`/grid/`Link`/`AppImage`/`MapPin`/`CITY_GRADIENTS` + dead imports (no double `<section>` — View owns it). Q3 + **mandatory `npm run build`** (new gate `67340ff49`). Kickoff `tasks/kickoff_prompt_Task_649_PopularLocations_Swap_View.md`. **Execute next — closes block B / homepage migration except HeroSearch (Phase-2).**
...
**Task numbering — last used: 643 (committed); 641 applied + closed; 642 approved + closed (company-field cluster done). 644/645 done (How-it-works). 646 (block A) + 647 (block C) approved + committed. 648 approved + committed (block B slice 1/2); 649 designed (execute next). Next free: 650. **Governance: `npm run build` exit 0 now mandatory for non-Q0 (`67340ff49`).**
