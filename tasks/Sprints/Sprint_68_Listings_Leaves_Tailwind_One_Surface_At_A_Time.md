...
| **779** | `/listings`: `ListingsFilterBar` → Mantine, route visibility moved to a `ListingsShellView` wrapper | **P2** | **Q3** | ✅ **APPROVED WITH NOTES** (Opus) 2026-09-02 — archived as one row with 780/780R. Its migration is the code that shipped; its two layout defects and its baseline-reuse gap were fixed under 780/780R. Kickoff `Sprint_68_kickoff_prompt_Task_779_ListingsFilterBar_Mantine.md`; session `docs/sessions/2026-09-02-task779-listings-filter-bar-mantine.md`. |
| **780** | `ListingsFilterBar` review rework: withdraw the production inset, absorb the `Indicator` overhang in the story harness | **P2** | **Q3** | ✅ **APPROVED WITH NOTES** (Opus) 2026-09-02 — archived. First attempt returned `NEEDS REVISION` (production-visible 12px inset for a Storybook-only condition; pre-final `V2`-`V6` transcripts; no ledger). Owner decision withdrew the inset; **780R** relocated the fix to the story harness and added the container-relative control that the previous instrument structurally lacked (plant P-A: `containerRelative` fails at 16/16 while every relative-only check passes). Ledger `docs/reviews/2026-09-02-task780R-…review-ledger.json`, 12/12 `VERIFIED`, gate `PASSED`/0. Kickoff `Sprint_68_kickoff_prompt_Task_780_ListingsFilterBar_ReviewRework.md` (+ S0 Revision 1). |
