...
**2026-06-13 — Task 319 (Epic II Phase 2 — notification locale-binding FIX, Model C) — OWNER-APPROVED; commit pending + manual migration required.** Core bug class fixed: new notifications store `template_id`/`template_params` (+ sq fallback) and render in viewer `useLocale()`; legacy = verbatim fallback (no backfill); `price_change` numeric params formatted at render; `emailLocale='sq'` preserved. tsc/lint/build/i18n/i18n-dynamic/schema-drift green; rendered matrix 28/28. Owner review: high quality, no rework. **Migration APPLIED** — owner ran `scripts/task-319-notifications-template-columns.sql`; live `notifications` confirmed 10 cols (`template_id text NULL` + `template_params jsonb NULL` present, `title`/`body` stay NOT NULL for sq fallback) → 319 deploy-ready, commit pending. **Follow-up Task 424 opened** — NotificationBell/NotificationCenter mobile <640 bottom-sheet (pre-existing P0, correctly out-of-319-scope via Sonnet STOP&ASK). Kickoff defect owned: 319's mobile-gate wording conflicted ("verify no h-scroll @320" vs "don't touch container") → split into 424. Kickoffs: `..._Task_319.md`, `..._Task_424.md`; session log `docs/sessions/2026-06-13-task319-notification-locale-binding-fix.md`. **Next: commit 319 → execute Task 424; then 320 → 323.**
