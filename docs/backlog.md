...
**2026-07-02 — Task 524 (`MantineTooltip`, P1.22) ❌ REJECTED by owner (rendered) → follow-up Task 526 opened; Tooltip UNCOMMITTED.** 524 built the primitive (hover/focus Mantine `Tooltip` §6k chrome ≥640 · tap→`ResponsiveBottomSheet` <640; caught+fixed Mantine's `events` focus:false a11y default). **Two owner rejections on the rendered result:** (1) desktop tooltip uses §6k `whitespace-nowrap` → long uk/it labels **clip/overflow off-screen** at it@680 — must WRAP (`multiline`+`maw≈260px`); (2) story shows only top/bottom — **left/right placements missing** (TailAdmin has Top/Right/Bottom/Left). **Task 526** fixes both in the same uncommitted working tree (desktop wrap-no-clip + all-4-placement story + §6k/§25 wrap-divergence note); orchestrator commits Tooltip (524+526) together after review. Root-cause note: my §6k said "nowrap" — corrected in 526. Kickoff: `tasks/Sprints/Sprint_39_kickoff_prompt_Task_526_TooltipWrapNoClip.md`. **Batch C NOT complete until 526 lands.** Prior: Task 523 (Drawer) committed `04cc304db`; clause 16 + Sprint 40 (Task 525 audit) still pending. **Next free: 527.**
...
**🟢 Sprint 39 (MM overlay Batch C) — COMPLETE, owner commits pending.** 509/510 (canonical Select) · 513 (Popover) · 514 (single-source `ResponsiveBottomSheet`) · 515 (DropdownMenu) · 516 (overlay trigger width) · 517 (drag-handle centering) · 518 (NavigationMenu) · 519/520/521 (Modal thread, committed + pushed) · 522 (§18.8 content-height fix, approved) · 523 (`MantineDrawer`) ✅ committed `04cc304db` · 524 (`MantineTooltip`, P1.22, implemented — see Last Session). Batch C (P1.18–P1.22) fully done → next is Batch D. Kickoffs under `tasks/Sprints/Sprint_39_*`.
