...
| 710 | reserved — Sprint 49, next up | **Meta-gate: an assertion that is `null` in every one of its target cells is a dead gate, not a pass.** Exactly how 573 died unnoticed for ~5 weeks. Same shape applies to `fullWidthControlsAtMobile` / `fullWidthButtonsAtMobile` / `popupBottomSheetAtMobile`, which are also `null`-by-default. **Scope addition (2026-08-05):** also gate the *exit-code* half — 709 persisted `EXIT_CODE=0` beside 4 genuine FAILs (a piped-capture artifact; 709-R proved the gate exits 1 unpiped). A gate that detects but does not fail CI is the same defect class. |
