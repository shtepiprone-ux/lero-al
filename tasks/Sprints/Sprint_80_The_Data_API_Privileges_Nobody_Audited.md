...
| **O80-6** | Task 883, after its approval and deploy: signed in, the bell loads, marks read and updates live; sign out with DevTools open → no `notifications?select` request after `logout`, no `[notifications] fetch failed` in Console. Exact steps: 883 kickoff §13.3. |
