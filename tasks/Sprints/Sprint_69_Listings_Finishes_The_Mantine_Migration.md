...
|---|---|---|---|---|
| **781** | `/listings` Mantine surface completion — status tabs, filter chips, action row and shell presentation, in four gated phases | **P2** | **Q3** | 🗄️ **ARCHIVED** (2026-09-03), historical verdict `PARTIALLY VERIFIED` under D69-11. **Reviewed 2026-09-04 to establish what remained:** of the findings carried to 782, six closed (F4·F7·F8·F9·F10·F11) plus F13; **F3·F5·F6·F12 remain open**, all four blocked on either a live route server + seeded DB or a complete rendered B/P matrix — closure kickoff **781R2** filed 2026-09-04 in this task's own kickoff file. **F1·F2 are unrecoverable** — the 2026-09-03 review was never persisted (no `docs/reviews/` artifact, no `docs/sessions/evidence/task781/`); only F3-F12 reached 782's kickoff §3.4. |
| **782** | Canonical Mantine dimension tokens, repo-wide raw-size sweep, and Task 781 review closure — five gated phases + revision round 1 | **P2** | **Q3** | 🗄️ **APPROVED — ARCHIVED (2026-09-03), F13-only closure.** This row was stale until 2026-09-04: it read `IMPLEMENTED — AWAITING ORCHESTRATOR REVIEW … owner visual review remains required` while the archive had carried the approval since 2026-09-03. Verified at review: F13 landed in commit `c082f56c8`, `ListingsSortBar.tsx` consumes the canonical `MantineCountButton` with no local `Badge`/`Indicator`, and the owner's manual Storybook pass covered all 24 cells (0/1/12 × sq/en/uk/it × 375/390px). Only remaining diff on that file is Task 784's `gap={2}`→`gap="micro"` token swap. |
...
| **785** | The eleven inert `styles`-prop media queries, and one orphaned label | **P1** | **Q3** | 🗄️ **APPROVED WITH NOTES — ARCHIVED (2026-09-04).** 13 inert blocks across 6 patterns → native responsive props; 31/31 rendered checks at a 639-vs-640 boundary; R8 inset at a 0px delta; `theme.ts` untouched; global set byte-identical; all six stories owner-accepted. Gutters deferred by owner. Three orchestrator kickoff defects recorded in §14. |
| **840** | The `/listings` filter leaves finish the migration: `FilterChoiceGroup`'s Tailwind `className` pass-through + `flex-col` sniff and `YearCombobox`'s `cn()` wrapper go; `FilterChoiceGroup`, `FilterRangeInputs`, `FilterRoomsRow` and `YearCombobox` are enrolled with own Stories — filed 2026-09-18 at Sprint 75 closure, owner instruction *"закривай спринт і заводь задачі на міграцію"* | P2 | **Q4** (critical flow: "Listings filter controls — leaf sub-components + shell") | ⬜ reserved — full text in `docs/backlog-reserved.md` |
