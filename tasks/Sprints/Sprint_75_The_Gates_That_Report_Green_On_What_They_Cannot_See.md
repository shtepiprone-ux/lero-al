...
| **832** | `check:click-shield` is blocking in CI and red on two defects no diff caused — the drawer scenario never opens, and one footer hit returns `null` | P2 | Q4 | **RESERVED** 2026-09-17 by Task 825 review 3 (`docs/sessions/evidence/task825/90_review2_click-shield.txt`, also `24`, `52`). ① `scripts/check-click-shield.mjs:179` trigger `header button:has(svg.lucide-heart)` matches nothing in all 16 drawer cells, so the AuthSheet drawer has had no coverage since the header migration (**787** is the likely cause, unproven). ② `[base] /it` mobile footer link `elementFromPoint` returns `null` (seen at 375 in Revision 0/1 and at 320 now). Fix: re-point the trigger to a live AuthSheet opener, prove it with a two-armed plant, and classify the null hit as a harness or product defect. Kickoff not yet written. |
...
822/797 share the design-tokens detector) · **743** independent. **826 · 827 · 828 · 829 · 830 · 831 · 832** have no kickoff yet (830 edits the same detector as 797 — after 797's commit; 831 edits the census writer 825 §16.3 works around — after 825's commit). **825** closed 2026-09-17 (review 3), so **831** is unblocked. **832** is independent.
