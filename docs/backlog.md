...
**2026-06-15 — Task 427 (Epic I, admin/owner full edit + any-status access) — REVIEWED → REWORK (routed back, NOT committed).** Orchestrator verified the diff against real code. Core logic (engine resolver, gateway auth, edit-route, admin-table derivation, owner-cabinet options, 1 i18n key) APPROVED. 4 gaps: (1) BLOCKER — privileged `to===from` returns `{ok:true}` no-op at `applyListingTransition.ts:231–234`; kickoff §2/Neg-flow require `invalid_transition` (owner decision: enforce literal AC); (2) AC5 delete closed declaratively — `deleteListingAction` confirmed status-gate-free (RLS ownership-only), needs proof trace in log (not a code fix); (3) `npm run build` not run (clause 9); (4) no rendered matrix — owner requires targeted uk@320/375/390 proof for the 2 density-changed surfaces (ListingPreviewDialog 5 buttons, StatusChangeControl 6 options). Rework kickoff: `tasks/Epics/Epic_I_kickoff_prompt_Task_427_REWORK.md`. **Next: Sonnet runs rework → orchestrator re-reviews.**
