...
**2026-06-15 — Task 246 (Epic DD.1 — admin can clear user-profile change history, gated + audited, per-row AND per-entity) — ✅ APPROVED after RPC rework, commit emitted (pending owner NATIVE verification + owner SQL run).** Two audited clear scopes added around the two `AdminUserProfile` history lists (`user_change_log`, `user_status_history`); listing-status history correctly excluded (no runtime surface). Atomicity blocker resolved: audit-insert + delete are now one atomic `clear_user_history()` PL/pgSQL call (SECURITY DEFINER, service_role-only EXECUTE) — a failed DELETE rolls back the audit INSERT (matches the negative flow: server error → no delete, no audit row). RLS/enforcement GREEN: `user_change_log`/`user_status_history` are service_role-GRANT-only (Task 275), so normal sessions are denied at the GRANT layer + `hasPermission('audit.clear_history')` gate (owner decision 7). Non-blocking follow-ups: schema-drift coverage for `history_clear_events` (candidate Task 431) + no-op race success toast. Log: `docs/sessions/2026-06-15-task246-admin-clear-history.md`. **Next: owner runs SQL Sections 1–3 in Supabase + native verification, then the emitted Task 246 commit.**
...
**Task numbering — last used: 431 (schema-drift guard: register `history_clear_events` — kickoff written, NOT yet executed). Epic II uses reserved 316–323.** **Next free (new): 432.**

**Task 431 (Epic DD follow-up — schema-drift guard registers `history_clear_events`) — 📝 KICKOFF READY, awaiting executor.** Mirrors Task 430: add `HistoryClearEvent: 'history_clear_events'` to `check-schema-drift.mjs` `INTERFACE_TABLE_MAP` (+ documented no-`.from()` coverage exception, like the Task 265 `search_vector` precedent — the table is written only via the `clear_user_history` RPC) and regenerate `schema-drift-check.sql` (32→33 tables, 305→315 cols). Kickoff: `tasks/Epics/Epic_DD_kickoff_prompt_Task_431_SchemaDriftHistoryClearEvents.md`. (The no-op race success toast from Task 246 is intentionally NOT bundled — separate Task 432 if wanted.)
...
| **DD — Admin Audit & History Hygiene** | **246** ✅ APPROVED after RPC rework (commit emitted, pending owner NATIVE verification + SQL run) | DD.1 admin can clear change history (gated + audited, per-row + per-entity); atomic `clear_user_history()` RPC; RLS via service_role-only GRANT (Task 275) + `hasPermission('audit.clear_history')`. Log: `docs/sessions/2026-06-15-task246-admin-clear-history.md` | [`Epic_DD_…`](../tasks/Epics/Epic_DD_Admin_Audit_and_History_Hygiene.md) |
