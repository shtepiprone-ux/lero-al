...
**Opened:** 2026-09-24 · **Status:** 🟠 **OPEN** · **Landed tasks:** 6 (872, 873, 875, 876, 878, 879) · **Kickoffs filed:** 0 · **Reserved:** 0 · **Moved out:** 874 → Sprint 78 (D78-7, 2026-09-24)
...
| **O81-7** | After 875 deploys, on lero.al: the Register widget on `/en`, `/it`, `/uk` and `/sq` matches O81-6, and one real registration or recovery request still verifies. |
| **O81-8** | 878's `OWNER VISUAL QA REQUIRED` matrix, before approval (kickoff §13.3): `HeaderView` `Default`/`SigningOut`, `HeaderActions`, `NotificationBellView`, `AuthSheet` `Login`/`Register`/`RegisterAgent`/`ForgotPassword`, and (review 1) `UserMenu`, `LocaleSwitcher`, `MobileNavDrawer`. **2026-09-24: all accepted except the bell's counter position (D81-8); after §17, re-check only the §17.5 tuples.** ✅ **Storybook part accepted 2026-09-24** (owner, after §18: *"Візуально тепер індікатор круглий"*). **Still owed after deploy:** the live checks in 878 §13.3 step 2. After deploy: the bell on lero.al, and the forgot-password success screen. |
| **O81-9** | 879's `OWNER VISUAL QA REQUIRED` matrix (kickoff §13.3, 7 rows, opened through the iframe URL at a set window width, because the toolbar viewport does not resize (799)), before approval. After deploy: the header at 390/1440, the drawer at 390 and one forgot-password success screen on lero.al. ✅ **Storybook part accepted 2026-09-25** (owner: *"Візуально все ок з цими сторісами"*, all 7 rows). **Still owed after deploy:** the live checks above. |
