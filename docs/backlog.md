...
| 718 | **KICKOFF FILED** — Sprint 52 | **`docs/design-system.md` §22.3 documents 7 `--z-*` tokens that do not exist.** `globals.css` defines none and says so at `:269-272`; no build defines one. Consuming `var(--z-sticky)` → invalid-at-computed-value → `z-index:auto`. Shipped into 2 chrome surfaces by 715, caught at review (F1). Table now carries a ⚠️ NOT-IMPLEMENTED banner; 718 decides **define the tokens or delete the table**, and adds the gate half — `check:design-tokens` exempts anything shaped like `var(--token)` without checking it resolves (adjacent to reserved **700**). **Also folds in 716-review F3** (`check-design-tokens.mjs:783`/`:797` still print "report-only, not blocking (Task 714)" and "715 owns the strict flip" — both false since 715 landed; 715's scope froze the file at one line so it could not absorb them). |
