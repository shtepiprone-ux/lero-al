...
| **810** | The rail becomes usable: controls, a visible scrollbar, equal card heights | P1 | **Q3** | **`NEEDS REVISION` 2026-09-10 — Revision 1 filed** (kickoff §16-§20). Revision 0 verified: AC1 scrollbar 4px→8px, AC2 control presence (comparator fired by a real planted arm), AC6 the D74-5 boundary, AC9 the grid freeze, all gates 0, `npm test` at the 790 baseline. Blocking: **R10** the D74-6 ladder keyed on the viewport, so the detail rail rendered 159px cards against the homepage's 249px and lost its controls (→ **D74-9**) · **R11** the probe never left `/` · **R12** grid mode unmeasured and the equal-height comparator never fired · **R13** AC3's paging arithmetic unmet at 5 of 9 widths (→ **D74-8**). **D74-7** closes the `offset` decision at 36. → [`Sprint_74_kickoff_prompt_Task_810_…`](Sprint_74_kickoff_prompt_Task_810_The_Rail_Becomes_Usable.md) |
