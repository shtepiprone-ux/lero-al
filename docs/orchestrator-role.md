...
## Demo-content source-of-truth gate (OWNER P0 — 2026-07-16) — MANDATORY in every story-touching kickoff AND review

The owner keeps demo/fixture content in Storybook stories (decision 2026-07-16 — stories are NOT wired to live
data), but a kickoff must never leave the demo VALUES to guesswork. Task 606's kickoff said "port the legacy
horizontal branch" without fixing the source of truth for the card's metrics, so Sonnet hand-authored `demoFeatures`
with a wrong icon (a bath icon on the "floor" chip) and an incomplete/mis-ordered set that drifted from the live
`getCardFeatures`/`propertyTypeSchema.ts`. That drift is MY kickoff failure, not the executor's. This is now a hard
gate on both sides of the loop:

- **Every kickoff I write** that touches a story/fixture rendering DEMO data for a component whose real props are
  derived from a source of truth (a schema, a domain engine like `getCardFeatures`, a token file, an enum/constant
  map) MUST enumerate the EXACT expected demo content WITH its source binding: the specific fields/rows, their order,
  and the exact icons/labels/values **cited to the source** — e.g. "feature row = `getCardFeatures` for apartment:
  rooms(`bed-double`) · bathrooms(`bath`) · area(`area`) · floor(`building` iconInCard), in that order; icons cited
  to `propertyTypeSchema.ts`, do NOT invent." Vague wording ("make it look like X", "port the legacy branch")
  WITHOUT the enumerated, source-cited demo spec is forbidden — it is the exact failure that produced the Task 606
  icon/metric drift.
- **The kickoff's acceptance criteria** MUST include a "demo values + icons/labels/order verified against the source
  of truth (`file:line`)" check, so the review verifies parity instead of letting a guessed fixture ship.
- **Every review I run** BLOCKS approval on a story/fixture whose demo metrics, icons, order, or labels do not match
  the cited source of truth — even if it "renders fine." A hand-authored fixture that diverges from the real
  engine/schema = route back.
- **Self-check every story-touching kickoff against this section before writing the file path to the owner** — same
  discipline as the mobile full-width / TailAdmin / presentational-split gates. The demo stays demo; the kickoff says
  WHAT the demo must be and WHERE its values/icons come from.
...
- [ ] **🔴 Demo-content source-of-truth (OWNER P0, 2026-07-16, after the Task 606 icon/metric drift):** any
      story/fixture rendering demo data for a source-of-truth-driven component has its demo metrics / icons / order /
      labels verified against the cited source (schema / domain engine like `getCardFeatures` / tokens / enum map),
      not hand-guessed. A fixture that diverges from the real engine/schema (even if it renders fine) = route back.
