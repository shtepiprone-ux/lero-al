...
Sprint 78 · P1 · QA profile **Q3** · Wave A, first task · **Status: 📝 KICKOFF FILED 2026-09-18 — READY FOR SONNET**
...
| Canonical states pattern | `src/design-system/mantine/patterns/MantineEmptyLoadingErrorState.tsx` + `Patterns/Mantine/EmptyLoadingErrorState` | Exists, enrolled. Its `loading` is a centered `Loader` and its empty block has `minHeight: theme.other.layout.emptyStateMinBlockSize` (200px). That is too tall for a top card, and the spec wants a **skeleton that repeats the card geometry**, not a spinner (§17.4). → reused **only** for the chart-area empty state in 845, not inside these compact cards. |
