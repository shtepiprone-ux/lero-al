...
**2026-07-06 — Task 555 (Sprint 41 / Epic MM Phase-2 · fix LocationCombobox icon/text overlap D1 + missing region placeholder D2) — ✅ APPROVED (orchestrator diff-level review + owner rendered evidence), commit emitted, pending owner NATIVE gate + run.** D1 root-caused in DevTools (not guessed): `input-chrome.css`'s `:not([data-with-left-section])` guard sat on the `<input>`, but Mantine puts the attribute on the `.mantine-*-wrapper` — so the guard ALWAYS matched, making the 16px start-padding override unconditional and clobbering Mantine's icon-reserved space. Fixed at the CANONICAL source (guard moved to `.mantine-TextInput-wrapper`/`.mantine-Select-wrapper`, both start+end padding — verified in the real file, `input-chrome.css:125–134`); regression-swept PropertyType/Year/AdminUsers consumers. D2: `LocationCombobox.tsx:163` adds `placeholder={tc('region')}` (reused `common.region`, zero new keys — verified in the real file). Owner-supplied rendered screenshots (320px + 1234px, UA) satisfy the §18.9 human-visual proof: icon↔text gap present, region placeholder shown, mobile full-width intact. Native gate re-run + file-integrity authoritative on the owner's PowerShell run (single-writer). Session: `docs/sessions/2026-07-06-task555-locationcombobox-icon-overlap-region-placeholder-fix.md`. **Unblocks Task 553/554 closure — next: DatePicker/PhoneField or FiltersPanel/HeroSearch.**
