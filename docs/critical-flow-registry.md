...
| **Signup confirmation → session/header** | signup confirm link → `/auth/confirm` → `/{locale}/auth/verified` | **446** | confirm link → state CONSISTENT: header authenticated iff session exists, and `/auth/verified` copy matches the real session state (no "you can use all features" while guest) | expired/used token → localized error + recovery CTA, no false success; confirm-but-sync-fail → deterministic state, never success-copy-while-guest | smoke: confirm → header/copy consistency + expired-token path | Slice 2 / **Task 446** | ❌ (live defect 2026-06-16: `/uk/auth/verified` shows success copy while header guest — A/B/C undecided, diagnose in 446) |
