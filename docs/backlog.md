...
**Task numbering — last used: 531 (kickoff written, unexecuted). Next free: 532.**

**🟡 Task 531 (Sprint 40 — control-family resting shadow `shadow-theme-xs`) — KICKOFF WRITTEN, unexecuted.** Last open Task 525 audit delta (TextInput "minor"): `theme.shadows.xs` is NOT overridden, so the input family + Button secondary + SegmentedControl + 2× `Paper shadow="xs"` render Mantine stock xs, not TailAdmin `shadow-theme-xs`. Fix mirrors Task 530: correct §5 to the exact zip literal `0px 1px 2px 0px rgba(16,24,40,0.05)` (currently an `≈` black approximation), then override `theme.shadows.xs` single-source (grep all 4 consumer groups; STOP-and-ASK only if a `Paper shadow="xs"` should stay on Mantine xs). Closes the Sprint 40 conformance queue. Kickoff: `tasks/Sprints/Sprint_40_kickoff_prompt_Task_531_ControlShadowXsConformance.md`.
