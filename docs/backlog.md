...
**Homepage → Mantine chrome migration ✅ COMPLETE** (A/646 inline chrome · C/647 skeletons+Featured heading · B/648–649 PopularLocations · D/650 HeroSearch container chrome — all approved + archived). Only intentional non-Mantine remnant: the HeroSearch Продаж/Оренда tab toggle stays a styled `Button` (owner declined `SegmentedControl`, look change). **Open follow-up (unowned):** Q0 doc-correction — `mantine-responsive-design-system.md §5` "Tailwind wins" is wrong (unlayered Mantine CSS wins; confirmed by Tasks 629 + 650).
...
**Task numbering — last used: 650 (approved). 641 applied + closed; 642 closed (company-field cluster done). 644–650 approved (homepage → Mantine chrome migration COMPLETE: how-it-works, inline chrome, skeletons, PopularLocations, HeroSearch container). Next free: 651. **Governance: `npm run build` exit 0 now mandatory for non-Q0 (`67340ff49`).**
