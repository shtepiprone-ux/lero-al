...
**Homepage → Mantine chrome migration ✅ COMPLETE** (A/646 · C/647 · B/648–649 · D/650 — all approved + archived).

**Designed — not yet executed:**
- **Task 651** — Q0 doc-correction: `mantine-responsive-design-system.md` §1/§2/§5 wrongly claim "Tailwind wins / `@layer mantine` separate / no conflict"; reality (confirmed Tasks 629 + 650): `@mantine/core/styles.css` is imported UNLAYERED (`layout.tsx:6`) so Mantine component CSS beats Tailwind utilities. Fix the 3 rows + add the `unstyled`/`Box`-not-`Paper` rule. Docs-only, Q0 (no build gate). Kickoff `tasks/kickoff_prompt_Task_651_Doc_Correction_Unlayered_Mantine_CSS.md`. **Execute anytime (independent).**
- **Task 652** — hero §6c unification (owner-approved redesign, mockup-confirmed 2026-07-20; reversed the earlier "keep tabs as Button"): Продаж/Оренда tabs → canonical §6c `SegmentedControl` (gray track/white pill) sitting **flush (0px)** on top of the search bar; search-bar surface white+`shadow-xl` → §6c gray track (`bg="gray.1"` + gray-2 border, no shadow), white input pills inside, brand coral Search kept. Mobile: tabs full-width 50/50 (`w`-responsive wrapper + `fullWidth`), Search full-width own row below. §6c colors + flush via Mantine props/`styles` (NOT Tailwind classes — unlayered-CSS rule). Q3 + build gate. Kickoff `tasks/kickoff_prompt_Task_652_HeroSearch_Unified_6c_SegmentedControl_And_Bar.md`. **Execute next.**
...
**Task numbering — last used: 650 (approved). 641 applied + closed; 642 closed (company-field cluster done). 644–650 approved (homepage → Mantine chrome migration COMPLETE). 651 (doc-correction) + 652 (hero §6c unification) designed. Next free: 653. **Governance: `npm run build` exit 0 now mandatory for non-Q0 (`67340ff49`).**
