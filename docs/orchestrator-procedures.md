...
**Corollary (791, 2026-09-05) — a 200, a title in the body and a green hydration gate are not proof that the route rendered, and the disproof was in the reviewer's own evidence directory.** Task 791's Revision 0 passed `npm run build`, `next start`, `curl` returning **HTTP 200 with the listing title present**, and `check:hydration` **PASS 5 / FAIL 0**. The review marked R11 `VERIFIED` on exactly those three. Meanwhile `docs/sessions/evidence/task791/start.log` — an artifact of that same run, sitting in the same folder — carried three occurrences of `⨯ Error: Functions cannot be passed directly to Client Components`: a `galleryLabels` object whose `counter` member is an arrow function was being passed from a Server Component into a `'use client'` pattern, and the route was erroring on **every** request. The next revision's rendered-evidence script surfaced it in minutes, and the fix was proven by a clean A/B (`start.log` has the error, `start-rev1.log` has none). **Three lessons, in order of how much they cost:** ① *the server's own log is primary evidence and must be read in its body, not summarised from the executor's report* — the reviewer opened `build.log` and `hydration.log` tails and took `start.log` from prose, which is "do not approve from a summary" committed by the reviewer; ② *a status code plus a string match proves the shell was flushed, not that the tree rendered* — Next.js streams, and `generateMetadata` emits `<title>` from a separate query, so both survive a body that never rendered; ③ *`check:hydration` measures console output inside a short wait window and is structurally blind to an error that arrives after it* — it is not a render gate and must never be cited as one. **The controls that would have caught this exist and were skipped: a rendered-evidence script with a `networkidle` wait, and reading the log.**

**Corollary (818/819, 2026-09-11) — `Get-Content -Raw` silently mojibakes a BOM-less UTF-8 file on Windows PowerShell 5.1, and it has now corrupted a shipped script and a shipped data file the same way.** Task 818: a restore step read `check-rendered-scope.mjs`'s pre-break content with `$content = Get-Content $path -Raw` (no `-Encoding utf8`) and wrote it back with `[IO.File]::WriteAllText` — PowerShell 5.1 decoded the BOM-less UTF-8 bytes through the system codepage on the read, so every em dash, `§` and box-drawing divider in the file's comments came back as CP1252-of-UTF-8 mojibake, and the write-back persisted it. Task 816 reproduced the identical failure on `scripts/rendered-scope-allowlist.json` (every `§` in a `reason` string became `Â§`) using the exact same pattern, while writing evidence *about* this very hazard. **Mitigation, now load-bearing across three tasks: never read a source or data file back through PowerShell's `Get-Content -Raw` without `-Encoding utf8` — read and write through Node (`readFileSync`/`writeFileSync`, both default to correct UTF-8) for any plant-and-restore probe, or pass `-Encoding utf8` explicitly if PowerShell must do the read.** Both incidents were caught only because the restore step also captured a `git hash-object` before/after witness — the hash mismatch is what surfaced silent corruption that every functional gate (JSON parse, the gate's own logic) missed, because `§` inside a comment or a `reason` string is never itself parsed. **A `git hash-object` witness on every plant-and-restore probe is therefore not optional evidence hygiene; it is the only check that catches this specific failure mode.**

**Corollary (BOM scope guard, 2026-09-11) — a correct byte-level repair is still unsafe if its candidate set is unbounded.** A BOM-cleanup script traversed all of `docs/sessions/` because it had no path guard; no unintended change remained, but the operation was capable of rewriting unrelated evidence. **Rule:** every mechanical multi-file write must first print an exact relative-path manifest and enforce it at the write boundary. A task that truly needs a directory-wide sweep names the root, extensions, exclusions, and expected count; any out-of-manifest candidate, duplicate, or count mismatch is `SCOPE GUARD FAILED` and performs no write. Node UTF-8 I/O (or explicit PowerShell `-Encoding utf8`), per-path pre/final hash witnesses, and a final diff are all required; encoding safety does not prove scope safety.

**Corollary (818, 2026-09-11) — a final gate-block transcript proves nothing was rebuilt after it, unless the transcript itself carries a hash tying it to the shipped content.** Task 818's original pass captured its full `§13.2` gate block, `npm run build` included, before running the AC16/AC18 planted probes; no artifact in that block recorded which exact version of the changed script produced it, so a later reader could not tell whether the retained transcript still described the shipped file. Task 819 closed the gap by adding `git hash-object` of every changed file to its own final gate block, and every task in this family since has followed the same shape. **Rule: the final validation gate block for any task that changes a script or data file must include the `git hash-object` of every changed file, captured in the same pass as the block's other commands — not asserted separately, and not left implicit in an earlier probe's witness.**

**Corollary (Sprint 75, 2026-09-11) — the sprint's transferable rule, now proven across four landed commands.** Sprint 75's whole thesis is that a check's scoping rule is also its blind spot, and a green result read without that scope is a false claim about the excluded set (Task 809's `/favorites`, `check:story-coverage` printing `34/34` while three unmigrated components sat on the page, outside the manifest it scopes to). The fix, applied identically four times this sprint, is **narrow the check, then print the narrowing on every run, so a green line can never be read as unscoped:** `check:rendered-scope` (Task 812/818) states its enrolled-subgraph scope and the tier-2/dynamic-import classes it cannot see; `check:surface-census` (Task 817) states its per-surface, transitive-tier-1-only scope; `check:surface-census:changed` (Task 819) states its diff-mapped scope, its changed-file/surface limits, and fails closed rather than silently skip when the mapping itself cannot resolve; `audit:design-system-patterns` (Task 816) states that its consumer counts are whole-`src` but its own-Story and enrolment checks still cannot see a consumer reached only through dynamic `import()`/`React.lazy()` or a directory the render graph never walks into. **When a kickoff narrows a detector to make it usable, state the class the narrowing excludes, in the kickoff, before the executor builds it — and require the shipped tool to print that same statement on every run, not only in the kickoff that designed it.**
