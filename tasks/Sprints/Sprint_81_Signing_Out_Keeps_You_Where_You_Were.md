...
| **O81-8** | 878's `OWNER VISUAL QA REQUIRED` matrix, before approval (kickoff §13.3): `HeaderView` `Default`/`SigningOut`, `HeaderActions`, `NotificationBellView` and `AuthSheet` `Login`/`Register`/`RegisterAgent`/`ForgotPassword`. After deploy: the bell on lero.al, and the forgot-password success screen. |
