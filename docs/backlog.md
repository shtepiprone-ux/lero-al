...
- **Task 648 (block B slice 1/2)** — created canonical `PopularLocationsView.tsx` + `Mantine/Primitives/PopularLocationsView` story (Title/SimpleGrid/Box + kept AppImage/CITY_GRADIENTS/hover-focus classes); does not touch `PopularLocations.tsx` (untouched, verified). typecheck/check:stories/check:story-coverage/check:i18n/mojibake all 0; Q3 rendered proof (7 captures: mobile/tablet/desktop × 4 locale samples + long-name truncate stress, both image+fallback branches). `mantine-migration-scope.json` deliberately not extended (unbroken prior-task precedent — P3 policy note, non-blocking). ✅ APPROVED WITH NOTES (orchestrator review 2026-07-20) — component/story correct, provenance full; gates all green: typecheck 0 / mojibake 0/1840 / **`npm run build` ✓ (75s, 40/40 pages)** (owner-native) + check:stories 122/0 + check:story-coverage + check:i18n 2206×4 (reviewer-reproduced). `bdrs="xl"` + fixture-URL self-catches sound. **Story-first slice 1/2; Task 649 wires it into `PopularLocations`.** Session: `docs/sessions/2026-07-20-task648-popularlocationsview-mantine-component-story.md`. Pending owner commit (648 paths only — the 9 uncommitted governance-policy files are the owner's separate WIP, EXCLUDED).
- **Task 649 (block B slice 2/2)** — refactor `PopularLocations.tsx` (keep the Supabase query/localization/href + null-guard) to render `<PopularLocationsView/>`, remove the inline legacy markup; rendered `/{locale}` proof (also the live before/after side-by-side deferred by 648). Not yet designed — depends on 648's component API. **Execute next (after 648 review).**
...
**Task numbering — last used: 643 (committed); 641 applied + closed; 642 approved + closed (company-field cluster done). 644/645 done (How-it-works). 646 (block A) + 647 (block C) approved + committed. 648 IMPLEMENTED - AWAITING ORCHESTRATOR REVIEW (block B slice 1/2); 649 planned (block B slice 2/2, PopularLocations). Next free: 650.
