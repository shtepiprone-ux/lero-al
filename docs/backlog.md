...
**2026-07-06 — Task 556 (Sprint 41 / Epic MM Phase-2 · PhoneField → Mantine/TailAdmin migration) — ✅ APPROVED (orchestrator review 2026-07-06: clean diff + all 8 native gates green + §18.9 uk@320/desktop eyeballed; 18 ambiguous cells all pre-existing non-PhoneField). ✅ COMMITTED `7e8d25c59`.** Legacy `Combobox`/`Input`/`Label` → `MantineCombobox`/Mantine `TextInput`/`InputLabel`; all phone LOGIC byte-identical (`phone.test.ts` 56/56 unaffected). **STOP-1 (owner: drop `size`, standardize h-11)** — 4 admin call sites (`AdminUserProfile`/`AdminUserCreate` × phone+whatsapp) had `size="sm"` removed; compact trigger via owner-cited `triggerWidth={{base:'7rem',sm:'7rem'}}` (=legacy `w-28`). **STOP-2 (owner: add `dropdownMinWidth` to `MantineCombobox`)** — new optional prop, desktop-dropdown-only, default-absent-byte-identical; `PhoneField` passes the legacy `240`. New `Mantine/Primitives/PhoneField` story + registry row + 3 RTL smoke tests + 2 planted-violation transcripts. Rendered gate 494/512 PASS, 0 FAIL, 18 pre-existing AMBIGUOUS (+16 clean). §18.9 human-visual proof at 6 breakpoint/locale combos + a live `AuthSheet` check confirming the country dropdown does not clip inside the registration Sheet. All 6 light gates green. Session: `docs/sessions/2026-07-06-task556-phonefield-mantine-migration.md`. **Next: DatePicker or FiltersPanel/HeroSearch themselves.**
