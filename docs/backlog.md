...
**Sprint 47 — Layout-shell de-hybrid** (`tasks/Sprints/Sprint_47_Layout_Shell_DeHybrid.md`), holding **673** (`FooterView`, implemented, awaiting review) → **706** (`HeaderView`). 🟠 OPEN. Opened 2026-08-01 because the owner selected 673 as the next Homepage kickoff and it does not fit 46's single dependency chain (691 → 702 → 695). **Two sprints are open in parallel; sequencing between them is an owner call.** New binding decisions: **D28** de-hybrid = mechanism-only, zero visual delta, keep `unstyled`, utilities → Mantine props + colocated `.module.css` (the 688 D16 pattern) — the first written form of what `backlog.md` had only summarised as "owner 2026-07-29, as in 672"; **D29** split footer-first.
...
| 673 | **`IMPLEMENTED - AWAITING ORCHESTRATOR REVIEW`** — Sprint 47 | `FooterView` de-hybrid done: 16 `className=` sites + 8 raw HTML elements → Mantine `Anchor`/`Text unstyled` + new `FooterView.module.css`; `SimpleGrid` replaces the raw grid `Box` (measured breakpoint match). All 16 enrolled cells md5-identical to a cross-validated pre-edit baseline; AC9 two-armed plant proved the comparator (Arm A flipped 16/16, Arm B restored 16/16). `check:design-tokens` still 28, 0 for `FooterView.tsx`. `build` exit 0. Official post-change full `--mantine-only` sweep was still running at report time — Opus should confirm its 16 `footerview` rows before final approval. Review `sessions/2026-08-01-task673-footerview-de-hybrid.md` |
...
| 706 | reserved, blocked on 673 — Sprint 47 | **`HeaderView` de-hybrid** — 11 `className=` sites, inherits 673's module pattern. Carries everything 673 does not: the P0 critical flow *Authenticated header hydration* (599/601), **3** live `.site-header` consumers (`check-header-id-parity.mjs:147` · `MantineRootProvider.tsx:34` · `task612-qa-listinggallery-lightbox-portal.mjs:57`), and Task 684's hardcoded **97px @320/375 · 65px @390/1024** measured off this header's own height. Owns all **5** `min-[390px]` token violations (`:110` ×3, `:128` ×2) → takes `check:design-tokens` 28 → 23. |
| 707 | **next free** | Assign to **Sprint 46** or **47** if it fits; otherwise open **Sprint 48** first. |

- **In flight, described once under "Open — needs action":** 661 · 665 (no sprint — both predate the rule) · 691 (Sprint 46) · 673 (Sprint 47, awaiting review — row above).
