...
|---|---|---|
| A — patterns | **843 → 844 → 845 → 846** | 843/844/845 approved. **🟢 Unblocked 2026-09-20: Task 861 is APPROVED and archived**, so R6's keyboard clause now holds and 846 is re-reviewed **unchanged** (no executor work owed). 846 and 861 share `messages/*.json` + `scripts/mantine-migration-scope.json`, so they land in **one combined commit** after 846's re-review. |
