...
|---|---|---|---|
| **691** | ❌ **`NEEDS REVISION`** 2026-08-12, Q3 — implemented and reviewed, **not landed, not approved**. **4 open P0** — `F-A` the removed `group` marker vs `appImageConfig.ts:66` and `F-B` the dropped compiled `@media (hover: hover)` guard are behavioural regressions; `F-C` (AC10 covers 4 of 160 tuples) and `F-E` (no retained build transcript) are blocking evidence gaps. Plus 2 open P1 (`F-J`, `F-K`) and 1 open P2 (`F-L`). **Remediation kickoff filed 2026-08-12: `Sprint_46_kickoff_prompt_Task_691R_MantineListingCardPattern_Remediation.md`** — re-entry suffix `691R`, closes with 691, consumes no registry number. Needs two owner actions before it can finish: a base-revision `git worktree` for the BEFORE capture, and OQ2 (`pointer: fine`). Ledger `docs/reviews/2026-08-12-task691-mantinelistingcardpattern-detailwind.review-ledger.json` — v4, gate `PASSED`/0, handoff `PROHIBITED`. Kickoff draft 2 2026-08-11 (`Sprint_46_kickoff_prompt_Task_691_…md`; draft 1 at the root of `tasks/` is ⛔ SUPERSEDED). ✅ **UNBLOCKED by D36** (owner, 2026-08-10), runs under a **no-increase** `/[locale]` First Load JS condition. **741 and 695 stay blocked** | `MantineListingCardPattern.tsx` de-Tailwind — **27 code sites** (`grep -c "className="` says 28; `:73` is JSDoc, the 702 §3.2 artifact again). Draft 1's "25 = 28 − 3 contracts" is void: **contract 1 exists in neither direction** (the `className?: string` prop at `:40` is read nowhere and no consumer passes it) and **contract 2 was dissolved by 702**. The 6 overlay utilities are **D35** family — reproduce compiled output, do not alias. **D34 is an open per-site question** (§3.6): the module is unlayered by design, so a utility that was *losing* today must not be promoted into one that wins. Comparator = the standing `--mantine-only` fail set as a **set** (`1164/1204, 18 FAIL`) **plus** `check:homepage-grid`. First task that can break **`check:css-vars`** — two tokens live only in this module. | 692 · 693 · 701 (all landed) |
