...
**2026-07-05 — Task 546 (ScrollArea primitive → TailAdmin, P1.26) — ✅ APPROVED (diff + rendered-manifest level), commit emitted, pending owner native gate + run.** New `theme.ts` `ScrollArea.defaultProps.scrollbarSize=6` (radius + track zero-override, geometrically/at-rest justified) + `scrollarea-chrome.css` gray-200 `#e4e7ec` thumb via the static `.mantine-ScrollArea-thumb` class (§6p, zip-cited `.custom-scrollbar`, native-vs-overlay reconciliation) + `Mantine/Primitives/ScrollArea` story (vertical/horizontal/empty; 6 keys × 4 locales, parity verified). Orchestrator-verified via Read-tool: theme block, chrome css, both chrome imports, story, §6p, §14.9.14, i18n parity (6×4). Rendered manifest `2026-07-05T09-14`: 430/448 PASS, 0 FAIL, 18 AMBIGUOUS (pre-existing Task 538 set); all 16 ScrollArea cells PASS incl. uk@320/375/390; planted-overflow 12/12 FAIL then reverted clean. Zero consumers migrated; swipe-scroll consumers pass explicit `scrollbarSize={0}` (unaffected). **Next:** P1.27 Slider · P1.29 Toast.
