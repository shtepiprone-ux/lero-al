...
| **792** | Detail-route chrome — `ListingBackButton`, `ListingStatusBanner`, `[slug]/loading.tsx`, the section wrappers — **plus the dead "similar listings" link** | P2 | Q2 | 🗄️ **APPROVED WITH NOTES — ARCHIVED** 2026-09-09, after Revision 1. AC8 was disproved by the task's own `i12-route-requests.txt` (`StatusCode 200` over a `NEXT_HTTP_ERROR_FALLBACK;404` body — the slug `11-mr7ucly4`, inherited from 791's session log, had stopped resolving); Revision 1 corrected the evidence to `shitet-gazonjere-ne-pogradec-mtu8u1lg` (`r2-body-*.txt`) and added the vitest AC5 required. Owner accepted the full live-route visual matrix. Notes P3: the `Group gap="sm"` asymmetry collapse, and `loading.tsx` as a Skeleton-only migration (R3's actual wording). Spawned **805**. → [`Sprint_71_kickoff_prompt_Task_792_…`](Sprint_71_kickoff_prompt_Task_792_Detail_Route_Chrome_And_The_Dead_Similar_Listings_Link.md) |
| **793** | The contact card becomes canonical Mantine, moves into the content flow, and takes its clearance with it | **P1** | Q3 | 🗄️ **APPROVED WITH NOTES — ARCHIVED** 2026-09-06 (ledger row filed; **subject to the owner's native §18.4 run** — a failing command, a leak count other than 131, or a mismatched visual reopens this as Revision 2) (Revision 1 verdict → kickoff **§18**). All five returned findings closed and **verified by the reviewer at source**: **F1** three story sections (`contactDisabled` + `closedListing` on `ListingContactPattern`, `ArchivedListing` on `ListingDetailView`); **F2** `contactLifecycleDisabled` now covers `sold`/`rented` per the owner's ruling, two label variables merged into one; **F3** the four-member cast replaced by a real type predicate `isListingNonActiveStatus` derived from the semantic layer, plus `status_banner_pending`/`_inactive` in all 4 locales (census 24/24 `OK`); **F4** `STYLES` widened to 6 rows; **P3** unconditional `dialogCoversViewport` in the Task 612 script. `check:locale-leak:mantine-only` exits 1 at **131** — accepted as a **pinned, arithmetically-reconciled** exception (131−107 = the 24 lines the one required new story export reproduces from Task **798**'s filed family); any later count above 131 unexplained by a new export is a regression. Notes N1 (stale R6/R11 rows in the session log), N2 (story label copy), N3 (live-route archived proof still owed — do **not** seed data). → [`Sprint_71_kickoff_prompt_Task_793_…`](Sprint_71_kickoff_prompt_Task_793_ListingContact_Canonical_Mantine_Card.md) |
| **808** | The detail route logs a React `key` warning on every render — find the real array, fix it, guard it | P2 | Q2 | 🗄️ **APPROVED WITH NOTES — ARCHIVED** 2026-09-10. R1's reproduction **refuted this kickoff's own §3.4**: `Children.toArray` assigns a positional key *and* stamps `_store.validated = 2` ("still needs a real key") on any child that had `key == null` and was never validated (`react.development.js:388-391`), and the reconciler warns on exactly that sentinel (`react-dom-client.development.js:26042-26049`) — both re-verified independently by the reviewer. §3.4 had read `filter-falsy-children.mjs` and stopped. Fix: two literal keys at the elements' creation sites in `ListingDetailView.tsx`, one production file, 2 insertions — causal, not a suppression, because it removes the `key == null` branch that stamps the sentinel; and `key` never enters `props` (`react-jsx-runtime.development.js:249`), so the DOM is unchanged by construction. Clean A/B on a real soft navigation: warning present before, absent after, same probe blob. Notes P3: the probe is not wired into `package.json` unlike its cited precedent `check:hydration`; the "update-reconciler-only" explanation for the soft-nav-only observation is an inference, not source-verified (the emitter is shared by both reconcilers); `favoriteSlot`'s half has no independent pre-fix capture; no `platform.txt` was retained, though three transcripts carry `C:/Claude_Code_Projects/lero-al`. → [`Sprint_71_kickoff_prompt_Task_808_…`](Sprint_71_kickoff_prompt_Task_808_Detail_Route_Missing_Key_Warning.md) |
| **794** | Gallery + lightbox: `GalleryStaticFrame` / `ListingGallery` inner composition, LCP mechanism preserved — **`LightboxView` moved to Sprint 75 Task 825 by owner decision 2026-09-16** | P2 | Q3 | ⬜ reserved — full text in `docs/backlog-reserved.md` |
