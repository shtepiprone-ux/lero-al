...
| **770** | 📝 **FILED 2026-08-26 — ready for executor** on base `06091ba1d` | **Level 3 — `@theme inline` runtime exit.** Migrate the fixed **42-pair / 79-use** twelve-file manifest (re-derived per file at filing) to nineteen new `--homepage-runtime-*` `:root` tokens of identical value, and ship `check:homepage-theme-runtime-deps` — thirteen fixed inputs, five ownership categories of which two block, D65-E's AppImage expected-zero arm, five `--verify-gate` outcomes. `@theme inline` stays byte-identical; eighteen non-manifest references still depend on it. Kickoff: `tasks/Sprints/Sprint_65_kickoff_prompt_Task_770_Homepage_Theme_Inline_Runtime_Exit.md`. |
