...
**2026-06-13 — Task 319 (Epic II Phase 2 — notification locale-binding FIX, Model C) — OWNER-APPROVED; commit pending + manual migration required.** Core bug class fixed: new notifications store `template_id`/`template_params` (+ sq fallback) and render in viewer `useLocale()`; legacy = verbatim fallback (no backfill); `price_change` numeric params formatted at render; `emailLocale='sq'` preserved. tsc/lint/build/i18n/i18n-dynamic/schema-drift green; rendered matrix 28/28. Owner review: high quality, no rework. **Two mandatory post-actions: (1) run `scripts/task-319-notifications-template-columns.sql` in Supabase SQL editor before deploy (→ Pending Action Items); (2) follow-up Task 424 opened — NotificationBell/NotificationCenter mobile <640 bottom-sheet (pre-existing P0, correctly out-of-319-scope via Sonnet STOP&ASK).** Kickoff defect owned: 319's mobile-gate wording conflicted ("verify no h-scroll @320" vs "don't touch container") → split into 424. Kickoffs: `..._Task_319.md`, `..._Task_424.md`; session log `docs/sessions/2026-06-13-task319-notification-locale-binding-fix.md`. **Next: run migration → commit 319 → execute Task 424; then 320 → 323.**

## Pending Action Items

| Item | Owner | Notes |
|------|-------|-------|
| 🗄️ **Run `scripts/task-319-notifications-template-columns.sql` in the Supabase SQL editor BEFORE deploying Task 319.** Adds `notifications.template_id text NULL` + `template_params jsonb NULL`. Until run, Task 319 producers/render rely on columns that don't exist in the live DB. | Owner | Task 319 migration (repo convention = `scripts/task-NNN-*.sql`, no `supabase/migrations/` dir). Reversible. |
...
**Epic II — Global i18n Hardening: Task 316 COMMITTED (`65a97a8cc`); Task 317 ✅ APPROVED (commit emitted, pending owner run); Task 423 ✅ APPROVED (commit emitted); Task 318 DONE (audit); Task 319 ✅ OWNER-APPROVED (commit pending + manual migration — `docs/sessions/2026-06-13-task319-notification-locale-binding-fix.md`); Task 424 OPENED (319 mobile-gate follow-up).** Task 317 output: `scripts/check-i18n-dynamic.mjs` + manifest + baseline + `docs/i18n-rules.md`. Task 423 hardened the same scanner's manifest/baseline validation — zero data edits. Task 318: `docs/governance-reports/2026-06-13-notification-locale-audit.md` (root cause named). Task 319: Model C render-time fix landed. **Task 424** (`tasks/Epics/Epic_II_kickoff_prompt_Task_424.md`): convert `NotificationBell`/`NotificationCenter` `w-80` anchored popup → §26.2 full-width bottom sheet at <640. Next queue: **run 319 migration → commit 319 → execute 424** → **320** (3 baselined `admin.support.user_status_*` keys) → **323** (CI wiring).
